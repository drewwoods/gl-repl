...
| Stage | Status | Notes |
|-------|--------|-------|
| **Stage 0** — Land the checks | ✅ Complete | `check-state-ownership` umbrella wired into `make test`; ratchets and allowlists live under `scripts/baselines/` and `scripts/allowlists/` |
| **Stage 1** — Make `ReplRuntimeState` real | ✅ Complete | All slices listed in the Stage-1 inventory live in `g_repl_state`; `repl_state_capture()` / `repl_state_restore()` round-trip in `test_repl_state.c` |
| **Stage 2** — Pilot value getters | ✅ Pilot done; broadly applied | `1ea317e` shipped the `help` slice as the canonical pilot. Most slices listed in the candidate set (`viewport`, `status`, `selection`, `variable_panel`, `variable_drag`, `profile_panel`, `pointer`, `camera`, `search`, `autocomplete`, `clipboard`, `scenes`, `code_panel`, `presentation`, `render`, `replay`) now return by-value; pointer-returning read getters survive only for `import_export` (returns `ReplImportExportView`), `editor_input` (returns `ReplEditorInputView`), and `variables` (returns `ReplVariableView`). The `check-views-by-value-snapshot` ratchet tracks remaining holdouts (baseline currently 0). |
| **Stage 3** — Convert UI-facing leaf state | ✅ Done at the read/write boundary | UI-facing leaf reads go through by-value getters (`repl_state_search()`, `repl_state_autocomplete()`, `repl_state_pointer()`, `repl_state_variable_panel()`, `repl_state_code_panel()`, `repl_state_help()`, `repl_state_status()`). UI files contain zero `_mut()` calls — `check-ui-no-repl-state-mut` is green; mutations route through `repl_actions` / `repl_command_store` / `repl_var_drag` / `repl_replay_*` helpers. `_mut()` is the canonical write path for owners and the controller, not for renderers. UI renderers still call `repl_state_*()` directly per-field — formalizing that into a single per-renderer `Ui*View` snapshot is Stage 7 work, not Stage 3. |
| **Stage 4** — Fix the cursor-pixel write with outputs | ⚠️ Action-helper variant landed; `Ui*Output` not | The direct write-through-view bug is gone: `ui_panels.c:249` now calls `repl_action_set_cursor_pixel(cursor_x, io_line_y)` which mutates through `repl_state_code_panel_mut()` inside `repl_actions.c`. `ui_autocomplete_panel.c` reads `cursor_px`/`cursor_py` from `repl_state_code_panel()` (a by-value snapshot). What is **not** done: the `UiPanelsOutput` struct + controller-actualization pattern from the plan. The current implementation has the renderer poke an action mid-frame instead of returning an output the controller writes back. The `scripts/allowlists/write-through-view.txt` baseline still lists `ui_panels.c:250-251` and should be re-baselined; `scripts/allowlists/cursor-px-encapsulated.txt` still allows `ui_panels.c` and `ui_autocomplete_panel.c` to touch cursor_px/cursor_py during migration. Decide whether the action-helper variant is the final shape or whether Stage 4 should still produce `UiPanelsOutput` for symmetry with future renderers. |
| **Stage 5** — Convert medium state slices | ⚠️ Read APIs done; renderer purity not | All medium slices listed in the plan return by-value snapshots (`ReplCameraState`, `ReplPresentationState`, `ReplRenderState`, `ReplReplayRuntimeState`, `ReplClipboardState`) or read-only view structs (`ReplEditorInputView`, `ReplImportExportView`). Workspace dir is exposed via `repl_state_workspace_dir()` (read) and `repl_state_workspace_set_dir()` (write). The Stage-5 "API shape, not scattering ownership" goal is met. What remains is structural rather than API-shape: UI renderers still pull these slices directly (e.g. `ui_panels.c` reads `repl_state_presentation()` ~10× for `show_vertex_indices` / `code_panel_layout`, and reads `repl_state_replay()` / `repl_state_render()` per frame), which Stage 7 would fix by routing them through a per-frame `Ui*View`. |
| **Stage 6** — Make capture/restore real consumers | ❌ Not started | Capture/restore exist and round-trip; `repl_undo.c` is not yet rebuilt on top of them. |
| **Stage 7** — Complete UI snapshot purity | ✅ Render boundary done; ✅ input-bridge leaves done (C-1 + C-2); ⚠️ top-level dispatch + cursor-pixel output deferred to C-3 | Every `ui_*_render*()` entry point now consumes `const UiRenderSnapshot *snap` built once per frame by `imrepl_ctrl_build_ui_snapshot()`. Phase C-1 + C-2 internalize the deferred-dispatch pattern across every input bridge in `ui_color_picker`, `ui_panels`, and `ui_menu_bar`: leaf handlers append to a stack-local `UiActionList`, top-level handlers drain it via `ui_action_dispatch_all()` before returning. Public `ui_*_handle_*` signatures are unchanged so `repl_editor.c` is untouched. C-3 (in `feature/push-architecture-ui.md`) covers the top-level dispatch lift, allowlist tightening, and the Stage-4 cursor-pixel `UiPanelsOutput`. |
| **Stage 8** — Collapse transitional state headers | ❌ Not started | `repl_state_views.h` and `repl_state_owners.h` are still public; `repl_state.h` is the compatibility shim. |
| **Stage 9** — Final cleanup and documentation | ❌ Not started | |

The remaining open items are: complete the cursor-pixel `Ui*Output`
actualization (Stage 4), make `repl_undo.c` consume `repl_state_capture()`
(Stage 6), and finish Stage 7's input-bridge cleanup (the Phase C work in
`feature/push-architecture-ui.md`). Stage 7's render boundary itself is
now snapshot-driven.
