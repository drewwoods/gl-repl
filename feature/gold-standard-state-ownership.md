...
| **Stage 7** — Complete UI snapshot purity | ❌ Not started | Most renderers still call `repl_state_*()` directly rather than receiving a `Ui*View`/`Ui*Output` pair. |
...
(Stage 6), and convert UI renderers to the snapshot/output pattern (Stage 7).
