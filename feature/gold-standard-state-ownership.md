...
| **Stage 7** — Complete UI snapshot purity | ✅ Render boundary done; input bridges deferred (Phase C) | Every `ui_*_render*()` entry point now consumes `const UiRenderSnapshot *snap` built once per frame by `imrepl_ctrl_build_ui_snapshot()`. New `check-ui-no-repl-state-read` guard wired into `make test`. UI files retain `repl_state_views.h` only for input-bridge helpers (`*_hit`, `*_rect`, color-picker press/motion, menu-bar hit-testing) which run from GLUT input handlers without a snapshot in scope — those move under Phase C. See `feature/push-architecture-ui.md` Phase B status. |
