...
| **Stage 7** — Complete UI snapshot purity | ✅ Render boundary done; ⚠️ input bridges in progress (Phase C-1 done, C-2 open) | Every `ui_*_render*()` entry point now consumes `const UiRenderSnapshot *snap` built once per frame by `imrepl_ctrl_build_ui_snapshot()`. New `check-ui-no-repl-state-read` guard wired into `make test`. Phase C-1 has landed `ui_action.h`/`ui_action.c` (UiAction tagged union + dispatch) plus the color-picker handler chain; remaining input bridges (`ui_panels`, `ui_menu_bar`) and `repl_editor.c` glue are tracked as Phase C-2 in `feature/push-architecture-ui.md`. |
