...
/* Returns: 0 on success, <0 on parse error. On success, *out contains cmd + normalized text. */
```

Update all parser call sites:
- Check return value (preserves status message setting in commit flow)
- Use output param `*out` to get both cmd and text

**Commit path** (repl_commit.c, repl_command_store.c):

- In `repl_commit.c` (lines ~308–317), after parsing with `repl_parser_parse_command_ctx(..., &parsed_line)`, pass both cmd and text to `repl_command_store_insert()` or `replace()` with text parameter.
...
- On scene load (line ~320): pass both to `repl_command_store_load(store, cmds, lines, count, edit_line)`
...
- On paste (line ~480): pass both to `repl_command_store_insert_many(store, at_idx, cmds, lines, count, flags)`
...
### 3c — Undo snapshot text sidecars (~1 day)

Undo snapshots currently hold only GLCmd arrays. Add text sidecars:
...
- **repl_undo.c:save()**: capture both cmds and lines from document before snapshot
- **repl_undo.c:restore()**: call `repl_command_store_load(store, cmds, lines, count, edit_line)` to restore both

### 3d — Test updates (~2 days)

Tests that construct GLCmd arrays or manipulate commands need text updates:

**tests/test_repl_editor.c**:
- ~40 assertion sites read from document via `document_cmds[i].source` → `repl_state_editor_buffer_line(i)`

**tests/test_scene_guides.c**:
- Test fixtures now pass lines alongside cmds to store load/insert APIs

**tests/** (general):
- Any test setup that builds cmd arrays should also build parallel text arrays and pass through store APIs
