...
typedef enum {
     ...
     * pure CPU, memoized per frame, and only paid when a helper pulls, so it
     * reads "--" for a backdrop that does not want bounds.
     *
     * Depth 1, a sibling of the ordered phases rather than a child of
     * `setup`, because the pull site belongs to the caller - a helper may
     * ask while placing lights or later while drawing, and PROF_RENDER3D is
     * the only bracket open across both. The nesting guard checks a claimed
     * parent is really open at both ends. */
