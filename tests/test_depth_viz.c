...
static void test_linear_perspective(void) {
    ...
    buffer_viz_depth_map(depth, 4, BUFFER_VIZ_DEPTH_LINEAR, &proj,
                         NULL, lum);
...
static void test_scene_two_depths(void) {
    Render3dProjectionDesc proj = make_desc(PROJ_PERSPECTIVE);
    BufferVizRange range = { 0.0f, 0.0f, 0 };
    float depth[3];
    ...
    buffer_viz_depth_map(depth, 3, BUFFER_VIZ_DEPTH_SCENE, &proj,
                         &range, lum);
...
static void test_scene_ema_overshoot_clamps(void) {
    ...
    buffer_viz_depth_map(depth, 2, BUFFER_VIZ_DEPTH_SCENE, &proj,
                         &range, lum);
...
static void test_scene_snap_on_jump(void) {
    ...
    BufferVizRange range = { 2.0f, 3.0f, 1 };
    float depth[2];
    unsigned char lum[2] = { 7, 7 };
...
static void test_split_maps_like_scene(void) {
    Render3dProjectionDesc proj = make_desc(PROJ_PERSPECTIVE);
    BufferVizRange range_scene = { 0.0f, 0.0f, 0 };
    BufferVizRange range_split = { 0.0f, 0.0f, 0 };
    float depth[2];
    ...
    buffer_viz_depth_map(depth, 2, BUFFER_VIZ_DEPTH_SPLIT, &proj,
...
static void test_ortho_passthrough(void) {
    ...
    buffer_viz_depth_map(depth, 3, BUFFER_VIZ_DEPTH_LINEAR, &proj,
                         NULL, lum);
...
static void test_degenerate_inputs(void) {
    ...
    buffer_viz_depth_map(NULL, 1, BUFFER_VIZ_DEPTH_LINEAR, &proj, NULL, lum);
    buffer_viz_depth_map(depth, 0, BUFFER_VIZ_DEPTH_LINEAR, &proj, NULL, lum);
    buffer_viz_depth_map(depth, 1, BUFFER_VIZ_DEPTH_LINEAR, NULL, NULL, lum);
    buffer_viz_depth_map(depth, 1, BUFFER_VIZ_DEPTH_LINEAR, &proj, NULL, NULL);
    AI("degenerate: output untouched by no-op calls", lum[0], 7);

    /* SCENE with no range state supplied falls back to LINEAR. */
    buffer_viz_depth_map(depth, 1, BUFFER_VIZ_DEPTH_SCENE, &proj,
                         NULL, lum);
 ...
 * out-of-range value has to bounce off buffer_viz_depth_render itself.
...
static void test_render_rejects_bad_modes(void) {
    ...
    buffer_viz_depth_render((BufferVizDepthMode)-1, &proj, 0, 0, 4, 4);
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_COUNT, &proj, 0, 0, 4, 4);
    buffer_viz_depth_render((BufferVizDepthMode)(BUFFER_VIZ_DEPTH_COUNT + 7),
                            &proj, 0, 0, 4, 4);
    /* OFF is in range but draws nothing. */
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_OFF, &proj, 0, 0, 4, 4);
    /* In-range mode, but no capture this frame / degenerate rect. */
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_LINEAR, &proj, 0, 0, 4, 4);
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_LINEAR, NULL, 0, 0, 4, 4);
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_LINEAR, &proj, 0, 0, 0, 0);
    ...
    buffer_viz_depth_capture(0, 0, 0, 0);
    buffer_viz_depth_render(BUFFER_VIZ_DEPTH_LINEAR, &proj, 0, 0, 4, 4);
