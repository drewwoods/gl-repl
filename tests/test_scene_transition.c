...
static float test_xn_opacity(int theme, SceneXnPhase phase, float elapsed) {
    (void)theme;
    if (phase == SCENE_XN_STEADY) return 1.0f;
    float dur = (phase == SCENE_XN_FADE_IN) ? TEST_IN : TEST_OUT;
    float p = (dur > 0.0f) ? elapsed / dur : 1.0f;
    if (p < 0.0f) p = 0.0f;
    if (p > 1.0f) p = 1.0f;
    return (phase == SCENE_XN_FADE_IN) ? p : 1.0f - p;
}
static float test_xn_elapsed_at(int theme, SceneXnPhase phase, float opacity) {
    (void)theme;
    float dur = (phase == SCENE_XN_FADE_IN) ? TEST_IN : TEST_OUT;
...
static const SceneXnReveal test_reveal = { test_xn_opacity, test_xn_elapsed_at };

int main(void) {
    SceneXnState s;

    /* rule 8: init = snap, fully shown, no animation. */
    scene_xn_init(&s, 8, &test_reveal);
    AT("init theme", s.current == 8);
    AT("init STEADY", s.phase == SCENE_XN_STEADY);
    AF("init opacity 1", scene_xn_opacity(&s), 1.0f);

    /* rule 5: same-value set while STEADY = no-op. */
    scene_xn_set(&s, 8);
    AT("same-value no-op STEADY", s.phase == SCENE_XN_STEADY);
    AF("same-value opacity unchanged", scene_xn_opacity(&s), 1.0f);
    ...
    scene_xn_tick(&s, 0.10f);                  /* 0.10/0.20 out -> 0.5 */
    AF("out half", scene_xn_opacity(&s), 0.5f);
    ...
    scene_xn_tick(&s, 0.20f);                  /* clock 0.30 -> opacity 0 -> adopt 5 */
    AT("adopt latest at 0 (skip 3)", s.current == 5);
    AT("now FADE_IN", s.phase == SCENE_XN_FADE_IN);
    AF("opacity 0 at crossing", scene_xn_opacity(&s), 0.0f);
    scene_xn_tick(&s, 0.30f);                  /* 0.30s in -> 1.0 */
    AF("fade-in complete", scene_xn_opacity(&s), 1.0f);
    AT("STEADY after fade-in", s.phase == SCENE_XN_STEADY);

    /* F1: return to current mid-FADE_OUT reverses, opacity-continuous. */
    scene_xn_set(&s, 7);
    scene_xn_tick(&s, 0.10f);                  /* OUT -> 0.5 */
    ...
    AF("reverse resumes from current opacity", scene_xn_opacity(&s), 0.5f);
    scene_xn_tick(&s, 0.15f);                  /* 0.15/0.30 -> +0.5 = 1 */
    AF("reverse completes", scene_xn_opacity(&s), 1.0f);
    AT("STEADY after reverse", s.phase == SCENE_XN_STEADY);

    /* rule 6: show-from-off skips the dead OUT. The controller calls
     * scene_xn_show (not scene_xn_set) when current == off index, so the new
     * theme fades IN from 0 with NO preceding FADE_OUT. */
    scene_xn_init(&s, 0, &test_reveal);        /* seeded to off */
    scene_xn_show(&s, 4);
    ...
    AF("show: opacity reset to 0", scene_xn_opacity(&s), 0.0f);
    scene_xn_tick(&s, 0.15f);                  /* 0.15/0.30 -> 0.5 */
    AF("show: fades in from 0", scene_xn_opacity(&s), 0.5f);
    AT("show: still FADE_IN mid-ramp", s.phase == SCENE_XN_FADE_IN);
    scene_xn_tick(&s, 0.15f);                  /* -> 1.0 */
    AF("show: fade-in completes", scene_xn_opacity(&s), 1.0f);
