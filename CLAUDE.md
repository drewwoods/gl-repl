...
| [`src/ui/support/cpuprof.c`](src/ui/support/cpuprof.c) | Compute-profile overlay surfaces, three separate floating panels: the section listing panel (per-section CPU, GPU, and Max columns; Max = worse of the two EMAs — deliberately not paired per-frame samples; see the NOTE at its computation), the FPS plot panel (10s/1m/10m series from the `prof_fps_*` history), and the section histogram panel (every depth-0 section's `prof_section_histogram` overlaid on one graph — additively blended fills plus straight-alpha silhouettes, log-scaled y so a section pinned in one bin doesn't flatten its neighbors). Its x axis is the max across series of each series' own 95th percentile (`ui_histogram_series_axis_bin`): a *per-series* percentile because the fixed 0..100 ms bins mean one startup stall would otherwise pin the axis at the overflow bin, while pooling the series would let the many sub-100 µs sections (all mass in bin 0) clip the slowest section's real spread. Trimmed samples surface as "+N off", never silently dropped |
| [`src/ui/support/cpuprof.h`](src/ui/support/cpuprof.h) | Profile / FPS / histogram panel render entrypoints + the pure `ui_histogram_series_axis_bin` axis policy. Ctrl+W cycles Off / Plot / Sections / Details; the listing and histogram panels both appear from Sections up, the FPS plot from Plot up |
...
| [`src/support/cpuprof.h`](src/support/cpuprof.h) | Profiling API (`prof_begin`, `prof_end`, `prof_frame_tick`, fixed 1024-bin timing histograms, etc.); no UI/app dependency — `#ifndef PROF_SECTIONS_PROVIDED` fallback makes it compile with no catalog (`check-cpuprof-standalone`). Declares [`prof_section_info()`](src/support/cpuprof.h#L50) (per-section label/depth/is_total), implemented per-binary |
