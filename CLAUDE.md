...
- **Web build**: `make web` needs `emcc` on `PATH` - per-shell
  ...
  `web-deps.sh`, no browser**. 74 of the 76 test binaries run there; the 2 in
  `WEB_TEST_EXCLUDE` each assert behavior the web build deliberately does not
  have (native audio device, hidden File menu), and
  ...
  exists. Its value over `make test` is the `__EMSCRIPTEN__` side of
...
[`glr_ctrl_display_frame()`](src/app/glr_ctrl.h#L263) drives each frame: rebuild autonormals + flat
...
scissor, clears no color/depth) → [`render3d_draw_scene()`](src/render3d/render.h#L129) (projection → user
...
[`Render3dState`](src/render3d/render.h#L88) layout change still needs a relaunch).
