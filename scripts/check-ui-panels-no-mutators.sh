#!/bin/bash
# Hard guard: src/ui/panels.c is hit-test-only.
#
...
# UiHit returned from ui_panels_hit_test. src/ui/panels.c carries no
...
#
# This guard greps src/ui/panels.c for any of the J2 acceptance symbols.
...
violations=$(grep -nE \
    ...
    src/ui/panels.c 2>/dev/null \
    | grep -vE '^[0-9]+:[[:space:]]*(/\*|\*|//)' \
    || true)

if [ -n "$violations" ]; then
    echo "ERROR: src/ui/panels.c references a deleted input-dispatch symbol." >&2
    echo "src/ui/panels.c is hit-test only after Phase J2.2; the controller" >&2
