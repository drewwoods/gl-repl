...
violations=$(grep -nE \
    'ui_panels_handle_code_panel_(press|click|drag|release)\b|ui_panels_handle_(scene_press|motion|mouse_release|escape)\b|ui_color_picker_(open|close|press|motion|release)\b|replay_handle_pin_clicked\b|handle_search_key\b|ui_menu_bar_(close|set_open_menu|activate_dropdown_item|note_search_opened)\b' \
