#!/bin/bash
# Hard guard: src/repl/executor.c must syntax-check with -DNO_POINT_PARAMETER
# without including glr_camera.h or any other app/UI header.
#
# The compile-time pitfall the guard catches: src/repl/executor.c used
...
if ! "$CC" -fsyntax-only -DNO_POINT_PARAMETER -DGL_SILENCE_DEPRECATION \
    ...
    echo "ERROR: src/repl/executor.c does not syntax-check with NO_POINT_PARAMETER=1." >&2
    echo "Step 7e moved the camera-distance fallback off direct glr_camera() calls" >&2
    echo "and onto a controller-installed callback. New code in src/repl/executor.c" >&2
