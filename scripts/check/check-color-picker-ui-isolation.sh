...
#
# Forbidden in `src/ui/subsystems/color_picker.c`:
...
targets=()
for candidate in src/ui/subsystems/color_picker.c; do
...
if [ -n "$violations" ]; then
    echo "ERROR: src/ui/subsystems/color_picker.c references a forbidden mutator or live state read." >&2
...
# helper).
sig_bad=0
for hdr in src/ui/subsystems/color_picker.h; do
    [ -e "$hdr" ] || continue
