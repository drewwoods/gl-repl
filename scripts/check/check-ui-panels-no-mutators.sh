#!/bin/bash
# Hard guard: src/ui/app/panels.c is hit-test-only.
#
...
# UiHit returned from ui_panels_hit_test. src/ui/app/panels.c carries no
...
#
# This guard greps src/ui/app/panels.c for any of the J2 acceptance symbols.
...
violations=$(grep -nE \
    'ui_panels_handle_code_panel_(press|click|drag|release)\b|ui_panels_handle_(scene_press|motion|mouse_release|escape)\b|color_picker_(open|close|handle_press|handle_motion|handle_release)\b|replay_handle_pin_clicked\b|editor_search_handle_key\b|ui_menu_bar_(close|set_open_menu|activate_dropdown_item|note_search_opened)\b' \
    src/ui/app/panels.c 2>/dev/null \
    | grep -vE '^[0-9]+:[[:space:]]*(/\*|\*|//)' \
    || true)

if [ -n "$violations" ]; then
    echo "ERROR: src/ui/app/panels.c references a deleted input-dispatch symbol." >&2
    echo "src/ui/app/panels.c is hit-test only after Phase J2.2; the controller" >&2
