...
#
#   scripts/docs-assets.sh [asset ...]      # default: all assets
...
#   - Supersampling: the software rasterizer has no MSAA and no
#     accumulation buffer, so scene-focused shots (code panel hidden)
#     render at 2x the target size via --window and downscale 50% —
#     4x supersampling. Full-UI shots stay 1x: the GLUT bitmap fonts
#     don't scale with the window, so downscaling would halve the UI
#     text size and make the code panel unreadable.
...
if [[ $# -gt 0 ]]; then WANTED=("$@"); else WANTED=("${ALL_ASSETS[@]}"); fi
want() {
    local a
...
# render <frames> <ss> <args...> — run one capture into $WORK/frames/.
# ss=1 renders at WxH; ss=2 renders at 2x for the 4x-supersampled shots.
# Extra env (GLR_EDIT_LINE etc.) is passed by exporting before the call.
render() {
    local frames=$1 ss=$2; shift 2
    rm -rf "$WORK/frames"; mkdir -p "$WORK/frames"
    FREEGLUT_CAPTURE_FRAMES=$frames FREEGLUT_CAPTURE_FILE="$WORK/frames/f" \
        "$BIN" "$@" --window $((W * ss))x$((H * ss)) --no-audio \
        >/dev/null 2>&1
}

# still <out.png> <frames> <ss> <args...> — keep the LAST rendered frame.
still() {
    local out=$1 frames=$2 ss=$3; shift 3
    render "$frames" "$ss" "$@"
    local last
    last="$(ls "$WORK/frames"/f-*.ppm | tail -1)"
    if [[ "$ss" -gt 1 ]]; then
...
if want hero; then
    still "$OUT/hero.png" $PLAIN_FRAMES 1 --example 26 --time 3
fi

if want first-triangle; then
    still "$OUT/first-triangle.png" $PLAIN_FRAMES 1 "$(stage_triangle)"
fi

if want vertex-overlays; then
    still "$OUT/vertex-overlays.png" $PLAIN_FRAMES 1 "$(stage_overlays)"
fi

if want wireframe; then
    still "$OUT/wireframe.png" $PLAIN_FRAMES 2 "$(stage_wireframe)"
fi

if want light-theme-studio; then
    still "$OUT/light-theme-studio.png" $PLAIN_FRAMES 2 "$(stage_lights)"
fi
...
# washed out. These two stay 1x — the lines ARE the subject.
if want grid-themes; then
    for theme in TRON RADAR AURORA SYNTHWAVE; do
        still "$WORK/grid_$theme.png" $FADE_FRAMES 1 \
...
if want backdrops; then
    still "$WORK/bd_CITYSCAPE.png" $FADE_FRAMES 2 \
        "$(stage_backdrop SCENE_BACKDROP_CITYSCAPE flat)"
    still "$WORK/bd_STARS.png" $FADE_FRAMES 2 \
        "$(stage_backdrop SCENE_BACKDROP_STARS)"
    still "$WORK/bd_SUNSET.png" $FADE_FRAMES 2 \
        "$(stage_backdrop SCENE_BACKDROP_SUNSET)"
    still "$WORK/bd_AURORA.png" $FADE_FRAMES 2 \
...
if want axes-compass; then
    still "$OUT/axes-compass.png" $FADE_FRAMES 1 "$(stage_axes)"
fi

if want view-mode-2d; then
    still "$OUT/view-mode-2d.png" $PLAIN_FRAMES 1 --example 1
fi

if want labels-orrery; then
    still "$OUT/labels-orrery.png" $PLAIN_FRAMES 1 --example 25 --time 4
fi

if want glu-tess; then
    still "$OUT/glu-tess.png" $PLAIN_FRAMES 1 --example 22
fi

if want glow-sprites; then
    still "$OUT/glow-sprites.png" $PLAIN_FRAMES 1 --example 16 --time 2
fi

if want transform-stress; then
    still "$OUT/transform-stress.png" $PLAIN_FRAMES 1 --example 20 --time 1
fi

if want variable-panel || want tune-badges; then
    still "$WORK/variable-panel.png" $PLAIN_FRAMES 1 "$(stage_tune)"
...
if want motion-blur; then
    ...
    still "$OUT/motion-blur.png" 12 1 "$(stage_blur)"
fi
...
if want xform-guide-still; then
    ( export GLR_EDIT_LINE=4
      still "$OUT/xform-guide-still.png" $PLAIN_FRAMES 1 "$(stage_guide)" )
fi
