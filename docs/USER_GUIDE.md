...
**F12** cycles forward through the 43 built-in examples, then any saved
...
**F5** cycles the scene backdrop: Off *(default)*, Stars, Cityscape,
...
| Ctrl+Up / Ctrl+Down | Previous / next function. Still jumps functions during replay and with help open (plain Up/Down keep changing replay speed / scrolling help) |
...
| Ctrl+[ / Ctrl+] (`Cmd+[` / `Cmd+]` on macOS) | Go back / forward through definition jumps. History clears when the source changes or another scene/workspace is loaded |
