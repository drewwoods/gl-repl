...
- **State has explicit owners.** [`ReplRuntimeState`](../src/repl/state.h#L18) is the REPL program runtime.
  [`EditorState`](../src/editor/state.h#L207) is the text-document session. [`UiState`](../src/ui/app/state.h#L20) is transient UI/session
  chrome. [`Render3dState`](../src/render3d/render.h#L101) is the render3d renderer's frame-to-frame
...
- **UI is view + hit-test, not a controller.** Renderers consume
  [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L92). Input handlers compute a [`UiHit`](../src/ui/core/hit.h#L60) and return.
...
| [`Render3dState`](../src/render3d/render.h#L101) | Per-renderer 3D stage state that must persist across frames: ortho reference distance, active ortho edge tracking, and the most recent zero-jitter projection descriptor | REPL program state, editor text/session state, UI chrome, saved user-scene slots, app-level presentation config |
| [`VariablePanelState`](../src/subsystems/variable_panel/variable_panel_state.h#L64) (peer) | Variable-panel visibility flag + slider drag transaction (var_idx, coarse, start_value, start_x, value_changed, final_value) | Editor text behavior, REPL grammar |
| `AssignPlotView` ([`src/subsystems/assign_plot/assign_plot.h`](../src/subsystems/assign_plot/assign_plot.h)) (peer) | Which assignment rows are plotted (up to MAX_ASSIGN_PLOT_SERIES, series 0 primary), capture rate (once / 1 Hz / frame), which X axis is in force, the presentation chips (expanded, log-Y request), and per series the decimated plot columns plus the running min/max/mean/stddev | Panel geometry and placement (UI), the flat program itself (REPL) |
| [`ReplayRuntimeState`](../src/subsystems/replay/replay_state.h#L97) (peer) | Replay state machine: PC, mode, speed, accum, fade speed, src_line_idx, total_flat_cmds, code expansion mode, normal display, focused-vertex label | Editor text behavior, REPL grammar |
...
- **`make repl-live-demo`** ([`tools/repl_live_demo/repl_live_demo.c`](../tools/repl_live_demo/repl_live_demo.c)) - the
  *composition* counterpart to `repl_demo`: a one-file host controller that
  ...
  scene's `// camera` block through a demo-local [`ReplExportCameraBridge`](../src/repl/export.h#L93), runs
...
- **`make editor-demo`** (`tools/editor_demo/`) - a generic
  ...
  **not** link `src/ui/app`: the demo proves the editor model can render
  ...
  [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L92), menu bar, app chrome, or [`UiState`](../src/ui/app/state.h#L20). The demo is
...
- **Root neutral helpers**: [`src/ui/core/gl_2d.h`](../src/ui/core/gl_2d.h) `gl2d_*` (2D primitives, the state/action mouse-chip grammar, and the two-arrow `gl2d_stepper_*` - the app's one "nudge a number by a step" control, shared by the inline numeric swatch, the find-bar match navigator and the variable panel's frame stepper), [`src/repl/transform_utils.h`](../src/repl/transform_utils.h)
...
- **Borrowed cross-module API types** - a header *referencing* a type
  ...
  [`ReplCompileContext`](../src/repl/compile.h#L180) / [`ReplCompiledChange`](../src/repl/compile.h#L132) in
  ...
  [`src/app/glr_ctrl.h`](../src/app/glr_ctrl.h); and [`VariablePanelViewState`](../src/subsystems/variable_panel/variable_panel_state.h#L25)
...
4. Update the ownership checks in the same change. The capture/restore /
   reset path for [`ReplRuntimeState`](../src/repl/state.h#L18), [`EditorState`](../src/editor/state.h#L207), and [`UiState`](../src/ui/app/state.h#L20) must stay in
...
| `src/app/glr_ctrl_router` | The `glr_ctrl_router_*` input helpers, split out of [`glr_ctrl.c`](../src/app/glr_ctrl.c): GLUT dispatch shims plus [`UiHit`](../src/ui/core/hit.h#L60) routing for replay, audio, config, save, camera, variable panel, swatches, scene press, and wheel zoom - each to its owning subsystem, ahead of the editor dispatch. Routing only: feature policy a key or click happens to trigger belongs to the domain module, not here |
| `src/app/glr_ctrl_cycle` | Scene / tutorial cycling policy behind F12 / F11, the Scene + Tutorials menu rows and the menu bar's Prev/Next steppers: the catalog walk, skip-on-load-failure reporting, origin restore, the parked example place and the retained tutorial index. `glr_ctrl_cycle_peek()` (the stepper tooltip's prediction) and `glr_ctrl_cycle_selects_tutorials()` (which catalog a step walks) live here so neither can drift from the walk they describe |
| `src/app/glr_ctrl_recovery` | Quit-time recovery persistence carved out of [`glr_ctrl_router.c`](../src/app/glr_ctrl_router.c): the deferred-quit flag SIGINT sets, what counts as user work worth rescuing (an unpromoted built-in example is not), and the two writers - the live scene to `QUIT_RECOVERY_FILE`, or every occupied slot to a recovery workspace. Shared by Ctrl+Q, File > Quit, SIGINT, window close and Open Workspace |
...
| `src/app/glr_config` | The keyed config vocabulary ([`GlrConfigKey`](../src/app/glr_config.h#L47)) and get/set/cycle API over those descriptors; `@cfg` persistence goes by key name, not table index, so reordering rows stays safe. `glr_config_set` tails into the tutorial runner's state-change notify |
...
| `src/app/glr_variable_panel_bridge` | Everything the variable-panel peer needs from the app, in both directions: the [`VariablePanelValueSource`](../src/subsystems/variable_panel/variable_panel_drag.h#L27) reader drag-begin grabs a declared name + value through, and the two writes a drag produces - motion applies the live value (and owns the drag's single undo snapshot), release rewrites the declaration row exactly once. That split is why the external-editor watcher emits one file write per gesture rather than one per pointer event. Keeps the peer linkable standalone in `variable_panel_demo` |
...
| `src/app/glr_prof` | Profile-section policy: the display table behind [`prof_section_info()`](../src/support/cpuprof.h#L171) plus which sections get GPU timer queries bracketed around them (GL-free sections stay CPU-only) |
| `src/app/glr_perf_hint` | Frame-rate watchdog for expensive non-default settings (accum passes, Post FX, line smooth). Ticks from [`glr_frame_begin()`](../src/app/glr_ctrl.h) with FPS + interval by value; the code-panel status strip names the heaviest culprit and offers a one-click step back. No GL, no UI. Session-scoped dismiss, no `GlrConfigKey`. Suppressed during tours and `FREEGLUT_CAPTURE_*` runs |
...
| `src/app/glr_clipboard` | The only TU that touches the *system* clipboard, and the installer of the [`EditorClipboardHostBridge`](../src/editor/clipboard.h#L88) the editor mirrors copy/cut through. Picks its backend at compile time: freeglut's `glutSetClipboardString`/`glutGetClipboardString` when `GLUT_HAS_CLIPBOARD` is visible (Cocoa implements them; other freeglut backends report an empty clipboard), `pbcopy`/`pbpaste` on the `make glut` Apple-framework build, none on web - where the DOM path in `glr_web_io` owns the clipboard instead. |
| `src/app/glr_source_document` | Full-app adapter binding the neutral `source_document` port (read view + insert/replace/load/clear/apply) to the [`EditorState`](../src/editor/state.h#L207) text buffer, so REPL pipeline TUs never reach into editor state directly |
...
| `src/app/glr_extedit` | External-editor sync behind `--watch` ([BYOE.md](plans/active/BYOE.md)). Follows `repl_active_scene_bound_path()` - the same file Ctrl+S writes, so the writer and the watcher cannot name different files - through a two-level gate: a `stat()` per frame, a read + hash only when the change token (nanosecond mtime + inode + size) moves, a re-parse only when the content hash does. Reloads are ATOMIC with `apply_cfg = 0` and `camera_apply = NONE`, so an external *text* edit is geometry and never a presentation reset. Holds a save that would land on a dirty input row or during a tutorial/tour, and decides the wait by comparing the document fingerprint rather than by hooking commit/cancel - an unchanged document means the row was abandoned, a changed one means the user committed and the external payload is dismissed. Undo policy splits on scene identity: a user scene gets one undoable clobber over a heap-backed history capture, an unedited example gets no push at all (`editor_undo_push_snapshot` is the transient auto-promotion hook). Stage 2.5 adds a **live WIP sidecar**: `<file>.wip`, the editor's unsaved buffer plus a trailing `// @cursor <row> <col>`, followed per keystroke rather than per save. The payload hash excludes the cursor line, so a cursor-only publication costs a stat and a hash and places the caret through the physical->document map the last content import cached ([`ReplSceneRowMap`](../src/repl/scene_load.h)); the named row is parked in the input line wherever it sits in the file when it is lexically incomplete, via the nonce-scoped `@cursor-hole` marker. One undo entry per session, not per keystroke. A sidecar present at bind time is a dead editor's unsaved work and is offered (`GLR_MODAL_CONFIRM_WIP_RECOVER`), never applied; one that disappears splits on the same identity D4 does. Web build inert |
| `src/app/glr_pointer_script` | Scripted synthetic pointer/keyboard engine. Two run kinds: env-driven capture (`GLR_POINTER_SCRIPT`, video recording; untimed-completion or absolute-timestamp, never canceled, no HUD) and menu-started **controlled tours** (untimed only; `glr_pointer_script_start_tour`) with a virtual clock, a `0.25×`-`16×` speed ladder, play/pause, immediate single-step, backstep (one whole-app baseline + prefix replay via `glr_tour_snapshot`), a final-caption Done linger followed by auto-close, and the [`GlrTourPlaybackView`](../src/app/glr_pointer_script.h#L159) HUD feed. Transport input arrives through `glr_pointer_script_handle_tour_key` / `_handle_tour_special` - the module owns which keys are transport, while the physical controller entry points own when to offer them and when to cancel. Symbolic point targets (`menu:`/`item:`/`sub:`/`pin:`/`scene:` resolved against live app layout, plus web-only `shell:` DOM controls) and the cursor/ripple/ring/caption overlay |
...
| `repl_apply` | The mutating half compile deliberately lacks: applies a validated [`ReplCompiledChange`](../src/repl/compile.h#L132) to the runtime arrays. Pure mutator - no status, no diagnostics (`check-no-set-status-in-compile-apply`) |
...
| `repl_gl_state_inspector` | Purely folds every state write in the generated `init()`/`display()` setup (including lights, camera/modelview, render toggles, and attribute-stack depth) plus the flat user stream to a source checkpoint, reporting explicitly touched state, its latest source, and a comparison basis - the OpenGL 2.1 initial values, or (via `repl_gl_state_report_rebase`) the same states' values at a second probe line, matched by name so the report reads as the differential between two source positions; light positions are folded through the active modelview exactly as fixed-function OpenGL stores them, and the inspector issues no GL calls. User glPushAttrib/glPopAttrib scope the fold via a masked snapshot stack (bit membership reused from `repl_attrib_bits`), on a virtual depth kept distinct from the generated display bracket |
| `repl_executor` | Narrow live-GL boundary that executes flat user geometry. **Takes a flat program and nothing else** - no source-text view and no diagnostic channel, so it renders only from baked `args[]` and cannot report on a line. That is structural, not a convention: `goto` was the sole command whose meaning lived in its source line rather than its args, and the last writer of a status message, so both inputs went with it. Owns command semantics, and therefore also the **background observation**: the cursor that emits `glClearColor` / `glColorMask` / `glClear` records what those clears established ([`ReplBackgroundObservation`](../src/repl/executor.h#L64)), so no second walk predicts it. Nothing outside may re-derive the frame background from the program |
...
not carry source text. Per-line text belongs to [`EditorState`](../src/editor/state.h#L207).
...
| Module | Role |
|--------|------|
| `editor_input` | Editor's pure text-document controller. Receives key/mouse events from `glr_ctrl` only after the controller has already filtered out non-editor concerns (replay, audio, config, save, camera, variable panel, scene press, scroll-wheel zoom). Mutates [`EditorState`](../src/editor/state.h#L207) directly: cursor, selection, scroll, search, autocomplete navigation, clipboard, undo. Also exposes hit-test predicates (`editor_input_point_in_code_panel`, etc.) and the [`EditorInputDispatchEffects`](../src/editor/input.h#L36) accumulation API that the controller consumes. `repl_editor.{c,h}` is deleted; this module is the sole dispatch boundary |
| `editor_commit` | Transaction boundary for commits: compile, undo snapshot, text-buffer write, REPL apply, dirty-state updates |
| `editor_state` | Owns [`EditorState`](../src/editor/state.h#L207): canonical per-line text buffer, active input, cursor, edit line, insert mode, **input-buffer selection anchor (`anchor_pos` on [`EditorInputState`](../src/editor/state.h#L52))**, line-range selection, search, autocomplete, scroll, undo/redo, transformers, highlights, virtual lines. The clipboard is a tagged union ([`EditorClipboardKind`](../src/editor/state.h#L107): EMPTY / LINES / INPUT_TEXT) so partial-line copy/cut and whole-line copy/cut share one slot. Single writer for editor buffer text |
| `editor_undo` | Undo/redo transaction rings that restore editor text and REPL command state together |
| `editor_navigation_history` | Bounded browser-style history of source locations created by go-to-definition jumps. Owns back/forward branching only; the app/REPL route resolves symbols. Source mutations and wholesale scene/workspace replacement invalidate the line-index history |
| `editor_clipboard` | Selection anchors plus copy/cut/paste payloads, including parallel text sidecars. Makes no platform calls: the OS clipboard reaches it through the optional [`EditorClipboardHostBridge`](../src/editor/clipboard.h#L88) that `glr_clipboard` installs - copy/cut mirror the internal payload outward, paste adopts the system clipboard only when something outside took it over, so an in-app round trip keeps its payload kind and block-aware line range |
| `editor_search` | One [`EditorSearchState`](../src/editor/state.h#L143) covering both find and replace fields: query, match tracking, row/char hits, next/previous navigation, the `whole_word` flag (a *matching* concern, so highlight and replace can't disagree), and the find/replace/word-chip focus ring |
| `editor_replace` | Find-bar replace: substitutes text across the **committed** buffer (never the live input row, so a half-typed line can't fail the operation), then hands the result to `repl_document_rebuild`. Pushes one undo snapshot per replace and rewinds the ring via [`editor_undo_ring_state_restore()`](../src/editor/undo.h#L119) if the rebuild is rejected, so a failed replace leaves no trace. Replace-current addresses its match by occurrence ordinal within the row, not char offset |
...
| `buffer_viz` | Peer subsystem ([`src/subsystems/buffer_viz/buffer_viz.c`](../src/subsystems/buffer_viz/buffer_viz.c)): framebuffer *inspection* - reads a buffer back and composites a false-colour view of it. [`depth_viz.c`](../src/subsystems/buffer_viz/depth_viz.c) provides linear / scene-relative / split-screen depth visualization; [`stencil_viz.c`](../src/subsystems/buffer_viz/stencil_viz.c) provides palette / ramp / split stencil visualization plus the raw histogram consumed by the legend. Each has a GL-free conversion core and a thin GL shell. render3d still *executes* the passes but is agnostic to them: the subsystem subscribes the neutral [`Render3dRenderConfig`](../src/render3d/render_types.h#L139) buffer hooks (`buffer_read_fn` / `buffer_pass_overlay_fn` / `buffer_resolve_overlay_fn`) through the controller, which owns the modes and the readback-capability mask |
| `call_depth_viz` | Peer subsystem ([`src/subsystems/call_depth_viz/call_depth_viz.c`](../src/subsystems/call_depth_viz/call_depth_viz.c)): colour-by-call-depth. Bins the flat program by `GLCmd.call_depth` (`call_depth_viz_scan`) and maps depth to a cool->warm ramp normalized over the observed max depth (`call_depth_viz_ramp_rgb` / `_build_table`). **Pure - no GL, no state, no capture**: unlike its `buffer_viz` shape-mate its input is the *program*, not the pixels, so there is nothing to read back. The controller resolves both products once per frame and hands the colour table to the executor (`ReplExecutionOptions.depth_tint_*`) and the counts to the legend |
...
| `repl_scenes` | Eight explicit user-scene slots, stable persisted filenames, transactional managed-workspace save/load, promotion, and scene-side command/text snapshots ([`SceneSnapshot`](../src/repl/scene_snapshot.h#L17) owns the copy/apply payload) |
| `repl_workspace_io` | Pure filesystem/naming mechanics under `repl_scenes`: strict `.glr-workspace` manifest parsing/commit, safe path joins, directory creation, and stable filename slugs |
| `repl_example_loader` | Built-in example loading and active-example tracking. Offers every line, from index 0, to `repl_camera_header` and `repl_doc_order`; there is no header region and no "N lines consumed" contract |
...
mutations, backed by [`glr_source_document.c`](../src/app/glr_source_document.c) over [`EditorState`](../src/editor/state.h#L207)) or, for
...
never read REPL runtime state, [`EditorState`](../src/editor/state.h#L207), or [`UiState`](../src/ui/app/state.h#L20) directly.
...
| `src/render3d/postprocess_filter` | Optional post-process pass: **chromatic aberration** (capture the rect to a texture, redraw channel-offset passes) or **vignette** (blended elliptical darkening; no capture). Pure fixed-function GL, dispatched on [`Render3dPostFilterMode`](../src/render3d/postprocess_filter.h#L18). Driven over the **3D viewport rect** once per frame by `src/render3d/render`; the *same primitives* are reused over the **whole window** by the app-level `glr_compositor` (Layer 6) |
...
| [`src/render3d/guides/geometry_guides.c`](../src/render3d/guides/geometry_guides.c) | Vertex/primitive guide rendering from a [`Render3dGuideSnapshot`](../src/render3d/guides/guides_shared.h#L53). The controller fills the snapshot's cursor args from the flat program (funcN-local resolution) before calling in - see [Architecture: Cursor Edit Guides](ARCHITECTURE.md#cursor-edit-guides) |
| [`src/render3d/guides/transform_guides.c`](../src/render3d/guides/transform_guides.c) | Transform-guide rendering from a [`Render3dGuideSnapshot`](../src/render3d/guides/guides_shared.h#L53) (REPL-aware) |
| `glr_camera` | Camera/view transform helpers - orbit/pan/zoom drag state machine. `glr_ctrl_router_handle_camera_mouse` drives input; render3d consumes final camera state through [`Render3dRenderConfig`](../src/render3d/render_types.h#L139). (Future `render3d_camera_controls` move is still possible if the render3d/viewport split lands.) |
...
| `ui_snapshot` | Defines [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L92), the read-only bundle passed to every UI renderer |
...
| `ui_text_panel` | Generic text-panel renderer and hit-tester over [`UiTextPanelSnapshot`](../src/ui/core/text_panel.h#L278); owns wrapping, row drawing, cursor/search visuals, and generic text hit mapping. REPL/editor-free, guarded by `check-ui-text-panel-pure` |
...
| `ui_color_swatch` / `ui_numeric_swatch` / `ui_view_mode_swatch` | Inline code-panel affordances: the color square on an editable color line (reads the app-owned [`UiTransformer`](../src/ui/app/editor.h), which is why it sits in `ui/app` and not the picker peer), the stateless numeric stepper the controller re-derives each frame (placement + step size only; the widget is `gl2d_stepper_*` in `ui/core`), and the animated 2D/3D toggle cell driven by the controller's projection blend. Render + hit-test only |
| `ui_variable_panel_view` ([`src/ui/app/variable_panel_view.c`](../src/ui/app/variable_panel_view.c)) | Bakes scene rect / statusbar inset / panel-at-top into the dependency-light [`UiVariablePanelView`](../src/ui/subsystems/variable_panel.h#L70), so the variable-panel renderer never touches [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L92), `ui/app/layout`, or `ui/app/state` and stays linkable from `{ui/core, config}` alone - the 2D analogue of `glr_ctrl` building [`Render3dRenderConfig`](../src/render3d/render_types.h#L139) |
| `ui_repl_code_panel` | REPL-aware adapter over `ui_text_panel`: builds rows from [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L92), editor buffer/virtual-line views, command metadata, tutorial fade, replay annotations, and color-transformer state; rewrites generic hits back to source-line targets. Owns the left aux column (vertex indices, auto-normal `auto` tag), whose width `ui_repl_code_panel_compute_text_x` reserves unconditionally - the column is painted at `text_x - 6 * FONT_W`, so reserving and painting it must never disagree. The status strip is a table of items in [`repl_code_panel_statusbar.c`](../src/ui/app/repl_code_panel_statusbar.c) - visual order within an align cluster is table order, overflow cull is a separate rank |
...
| `ui_variable_panel` | Renderer for the variable-slider panel (the panel chrome - the *peer subsystem* owns drag/visibility state). Input returns `UI_HIT_VARIABLE_SLIDER`, or `UI_HIT_VARIABLE_TIME_STEP` (item_idx +1/-1) for the frame stepper drawn on the clock row - tested *before* the row, since it sits inside one. Its gutter is reserved on every row so all tracks share an x-extent |
...
| [`src/ui/subsystems/buffer_viz_legend.c`](../src/ui/subsystems/buffer_viz_legend.c) | **Feature-UI** (buffer_viz peer): colour-key legend panel in the scene rect's top-left corner - swatch + value + count per listed value, plus a total row. **Two producers share it**: stencil values (pixel counts, plus an always-retained background (0) row) and call depth (flat-command counts, `row_prefix = "d"`, no zero row - depth 0 is an ordinary value). The corner is single-tenant, so the controller picks one per frame and stencil wins. Pure renderer over the controller-built `UiBufferVizLegendView`; the subsystem publishes raw per-value counts ([`buffer_viz_stencil_histogram()`](../src/subsystems/buffer_viz/stencil_viz.h#L92)) and `glr_ctrl_build_buffer_viz_legend_view()` picks the top-N rows, because "which rows are worth showing" is presentation policy, not data. Render-only - no hit-test |
| `ui_gl_state_panel` | Floating four-column OpenGL-state popup table (state/current/basis/latest source, plus pure hit-test + scroll geometry) over the controller-built view of `repl_gl_state_inspector`'s report. The basis column's header is solved text, not a constant - it names the pinned basis line when the report carries one. Two independent folds: the header chip adds the basis/source columns, the title-row chip adds the generated-setup rows the report partitions off (`user_row_count`), which stay folded by default |
...
| `ui_assign_plot_panel` | Assignment-value plot renderer (lives at [`src/ui/support/assign_plot.c`](../src/ui/support/assign_plot.c)). Signed Y with no forced zero baseline, linear by default (an opt-in log axis: plain log10 for positive data, symmetric signed log10 for anything crossing zero) - the opposite call from the log-spaced section histogram next door, because assignment values are unitless and a variable living in [100, 101] still has to show its shape. Mouse-only controls, following the state/action chip grammar in `ui/core/gl_2d.h`: rate, lin/log and 1x/2x state chips (value in a well) plus the `[reset]` and `[x]` action chips - plus a hover-only legend, past one series, selecting whose statistics the panel reports |
...
| `tour_presence` | **Feature-UI** (controlled tours): the whole-window ambient layer - four gradient amber edge bands with a solid hairline, plus an entry title card naming the tour and its exit key. Pure over a [`GlrTourPresenceView`](../src/app/glr_tour_presence.h#L53); holds no state and hit-tests nothing. Drawn LAST in the host band, after the compositor post-process, so a user's own whole-frame Post FX cannot paint over the chrome that says which mode the app is in. Deliberately NOT routed through the Post FX effect enum - that is a user setting a tour may itself be demonstrating, and the two capture-based effects would read back the frame every frame and smear the code panel being read |
| `tour_hud` | **Feature-UI** (controlled tours): collapsible top-of-scene transport HUD. Compact mode shows name + expand affordance; expanded mode adds state / speed / step / source line + progress + controls. Reads only `snap->tour` ([`GlrTourPlaybackView`](../src/app/glr_pointer_script.h#L159)) under the `tour_ui_*` prefix and returns `UI_HIT_TOUR_HUD`; the controller routes that passive hit to transport-owned expand/collapse state. Separate from the bottom `replay_ui_hud` so both can show at once |
...
| `src/repl/scene_load` (header only) | The reader's *policy*, as [`ReplSceneLoadOpts`](../src/repl/scene_load.h#L127) rather than as four things inferred from context: source format (which decides canonical-order checking, previously sniffed off the diagnostic label - so a catalog entry named "Torus Knot" silently lost it), TOLERANT vs ATOMIC failure policy (ATOMIC aborts at the first rejected line and restores a [`SceneSnapshot`](../src/repl/scene_snapshot.h#L17) taken at begin-load), and whether `@cfg` / `@camera` reach live state at all. Implemented in `src/repl/import`; the `repl_export_load_from_*` trio are wrappers carrying today's defaults. The metadata pair exists for the external-editor watch path, where an inbound *text* edit is geometry and must not reset presentation |
...
| `repl_cfg_baseline` | Neutral cfg bag/bridge and `// @cfg <slug>` parser: owns [`ReplConfigBag`](../src/repl/cfg_baseline.h#L35), [`ReplConfigBridge`](../src/repl/cfg_baseline.h#L50), and `repl_config_extract_slug` for export/import, scene snapshots, and tutorial baselines |
...
flowchart TB
    ...
    ui["<b>5. 2D UI</b> (render + hit-test)<br/><b>ui/core:</b> text_panel · text_layout · text_search ·<br/>tabbed_overlay · gl_2d · hit · theme · metrics<br/><b>ui/app:</b> panels · menu_bar · scene_tabs · repl_code_panel ·<br/>repl_code_panel_statusbar · layout · overlay_layout ·<br/>autocomplete_panel · gl_state_panel · command_description_panel ·<br/>color/numeric/view_mode swatches · variable_panel_view<br/><b>ui/subsystems:</b> color_picker · variable_panel ·<br/>replay_hud · tour_hud · buffer_viz_legend<br/><b>ui/support:</b> cpuprof · memprof · assign_plot · console panels<br/>(snapshots in, UiHit out - never mutates)"]
...
- Source text crosses the REPL/host boundary through the
  ...
  [`EditorState`](../src/editor/state.h#L207)-backed adapter; the editor remains the single
  underlying writer.
...
flowchart LR
    ...
    subgraph peers["2b. Peer subsystems (own state + controller)"]
        ...
        scalldepth["src/subsystems/call_depth_viz/<br/>call_depth_viz.c<br/>colour-by-call-depth ramp (pure, no GL)"]
    ...
    subgraph ui_layer["5. 2D UI rendering + hit-test"]
        ...
        uibvlegend["src/ui/subsystems/buffer_viz_legend.c<br/>colour-key legend panel (feature-UI)"]
    end
...
| New 3D REPL-aware overlay | `render3d_*`, consuming snapshots/configs from [`Render3dRenderConfig`](../src/render3d/render_types.h#L139) |
...
| New editor session state | [`EditorState`](../src/editor/state.h#L207) slice |
