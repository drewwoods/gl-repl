...
- **Legacy GL/eval domain types** in `src/repl/` (cross-domain,
  deliberately un-prefixed): [`GLCmd`](../src/repl/command.h#L99), [`CmdType`](../src/repl/command.h#L37), [`ExprVar`](../src/repl/eval.h#L136), [`ExprCtx`](../src/repl/eval.h#L143),
...
| `src/app/glr_ctrl_router` | The `glr_ctrl_router_*` input helpers, split out of `glr_ctrl.c`: GLUT dispatch shims plus [`UiHit`](../src/ui/core/hit.h#L51) routing for replay, audio, config, save, camera, variable panel, swatches, scene press, and wheel zoom — each to its owning subsystem, ahead of the editor dispatch |
| `src/app/glr_ctrl_view_transition` | 2D/3D view-mode transition state machine carved out of `glr_ctrl.c`: the projection blend + camera easing when View mode toggles. Camera + presentation state only (no scene/ui/repl/editor), ticked by the controller |
...
| `src/app/glr_config` | The keyed config vocabulary (`GlrConfigKey`) and get/set/cycle API over those descriptors; `@cfg` persistence goes by key name, not table index, so reordering rows stays safe. `glr_config_set` tails into the tutorial runner's state-change notify |
| `src/app/glr_color_picker_bridge` | Installs the `ColorPickerHostBridge` (document read/write + viewport + code-panel rect) the color-picker peer reads through, so that peer links standalone in `color_picker_demo` |
| `src/app/glr_prof` | Profile-section policy: the display table behind `prof_section_info()` plus which sections get GPU timer queries bracketed around them (GL-free sections stay CPU-only) |
...
| `repl_command_store` | Low-level [`GLCmd`](../src/repl/command.h#L99) array mechanics only: insert, replace, delete, load. No text-buffer writes |
...
[`GLCmd`](../src/repl/command.h#L99) is a parse-result record: type, args, flags, provenance. It does
...
| `ui_variable_panel_view` ([`src/ui/app/variable_panel_view.c`](../src/ui/app/variable_panel_view.c)) | Bakes scene rect / statusbar inset / panel-at-top into the dependency-light `UiVariablePanelView`, so the variable-panel renderer never touches [`UiRenderSnapshot`](../src/ui/app/snapshot.h#L71), `ui/app/layout`, or `ui/app/state` and stays linkable from `{ui/core, config}` alone — the 2D analogue of `glr_ctrl` building [`Render3dRenderConfig`](../src/render3d/render_types.h#L135) |
