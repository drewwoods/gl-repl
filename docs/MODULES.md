...
- **Legacy GL/eval domain types** in `src/repl/` (cross-domain,
  ...
  [`TessVertex`](../src/repl/executor.h#L107), [`FlatCmdLocalVars`](../src/repl/flatten.h#L43), [`FlatProgramView`](../src/repl/flatten.h#L58),
...
| `repl_executor` | Narrow live-GL boundary that executes flat user geometry. **Takes a flat program and nothing else** - no source-text view and no diagnostic channel, so it renders only from baked `args[]` and cannot report on a line. That is structural, not a convention: `goto` was the sole command whose meaning lived in its source line rather than its args, and the last writer of a status message, so both inputs went with it. Owns command semantics, and therefore also the **background observation**: the cursor that emits `glClearColor` / `glColorMask` / `glClear` records what those clears established ([`ReplBackgroundObservation`](../src/repl/executor.h#L59)), so no second walk predicts it. Nothing outside may re-derive the frame background from the program |
