...
| `src/app/glr_tours` | Built-in guided-tour catalog behind the Tours menu — file-backed like the example scenes (`tours/*.pointer` + `tours/catalog.ini`, compiled in by `scripts/gen_tours.py`), played as completion-driven `glr_pointer_script` sequences and authored entirely with symbolic targets so they play at any window size; any real key/click cancels (intercepted in `gl_repl`'s GLUT callbacks) |
