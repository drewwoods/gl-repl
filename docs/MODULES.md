...
| `src/app/glr_pointer_script` | Scripted synthetic pointer/keyboard engine: env-driven capture mode (`GLR_POINTER_SCRIPT`, video recording) plus runtime-started mode with auto-stop (`glr_pointer_script_start_lines`), and the cursor/ripple/ring/caption overlay |
| `src/app/glr_tours` | Built-in guided-tour catalog behind the Tours menu — named pointer scripts (menus, editing, camera walkthroughs) played through `glr_pointer_script`; any real key/click cancels (intercepted in `gl_repl`'s GLUT callbacks) |
