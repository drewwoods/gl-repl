...
| `repl_executor` | Narrow live-GL boundary that executes flat user geometry. Owns command semantics, and therefore also the **background observation**: the cursor that emits `glClearColor` / `glColorMask` / `glClear` records what those clears established ([`ReplBackgroundObservation`](../src/repl/executor.h#L65)), so no second walk predicts it. Nothing outside may re-derive the frame background from the program |
