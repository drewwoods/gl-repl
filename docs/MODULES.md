...
| `src/app/glr_tours` | Built-in guided-tour catalog behind the Tours menu — named pointer scripts (menus, editing, camera walkthroughs) played through `glr_pointer_script`, authored entirely with symbolic targets so they play at any window size; any real key/click cancels (intercepted in `gl_repl`'s GLUT callbacks) |
