...
> `src/repl` interpreter — the [`GLCmd`](../src/repl/command.h#L109) record and provenance, the
...
assignments against the current bindings, and stores resolved [`GLCmd`](../src/repl/command.h#L109) records
...
[`GLCmd`](../src/repl/command.h#L109) is a pure parse-result struct: `type`, `args[]`, validity / vars
...
[`ReplRuntimeState`](../src/repl/state.h#L18). The parser returns both the [`GLCmd`](../src/repl/command.h#L109) and the canonical
...
membership predicate is [`repl_cmd_is_glut_solid()`](../src/repl/command.h#L245) in [`src/repl/command.h`](../src/repl/command.h)
...
[`glr_ctrl_display_frame()`](../src/app/glr_ctrl.h#L153) (snapshot/follow-scroll → render3d render →
...
6. **Round-trip: export / import / reformat.** Export can often emit canonical
   source text unchanged, but import must rebuild the same [`GLCmd`](../src/repl/command.h#L109) through
