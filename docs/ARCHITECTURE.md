...
Render3d modules may consume [`FlatProgramView`](../src/repl/flatten.h#L59), [`CmdType`](../src/repl/command.h#L48), and other
...
flowchart TD
    ...
    subgraph loop["for each accumulation sample"]
        direction TB
        p1["prepare Render3dFrameRenderContext<br/>(derived camera world pos & basis)"] --> p2["apply projection (render3d-local jitter)"]
        p2 --> p3["set up baseline lighting"]
        p3 --> p4["execute user geometry<br/>(narrow execution boundary)"]
        p4 --> p5["optional post_fill_fn<br/>(controller's replay-fade overlay)"]
        p5 --> p6["buffer_read_fn hook<br/>(depth/stencil inspection)"]
        p6 --> p7["render host chrome<br/>(backdrop, grid, axes, orbit target)"]
        p7 --> p8["buffer_pass_overlay_fn hook<br/>(sparse under-guides overlay)"]
        p8 --> p9["post_overlays_fn hook<br/>(geometry-reporting overlays)"]
        p9 --> p10["accumulate sample if accumulation AA active"]
    end
    p10 -.->|next sample| p1
    p10 --> r1["post_resolve_overlays_fn hook<br/>(bitmap-text labels)"]
    r1 --> r2["buffer_resolve_overlay_fn hook<br/>(full-rect viz overlays)"]
    r2 --> r3["post-process filter pass"]
```
...
prefix whose PC has not reached the clear simply publishes nothing known.

Under accumulation every sample writes and the last known one wins. That is an
**endpoint approximation, not the resolved background**: the samples are summed
at 1/N and `glAccum(GL_RETURN)` presents their weighted average, so where a
clear color moves *within* a frame the pixels show the mean while the retained
value is the final sample. The two coincide for AA and camera blur (every
sample re-bakes the same clear color) and diverge only for time blur over an
animated `glClearColor` - bounded by the parser's cap on clear-color
components. The endpoint is taken deliberately: it is the sample baked at the
true frame time, and weighted background averaging was scoped out of v1.
...
> `src/repl` interpreter - the [`GLCmd`](../src/repl/command.h#L127) record and provenance, the
...
assignments against the current bindings, and stores resolved [`GLCmd`](../src/repl/command.h#L127) records
...
| Validation / compilation (pure, returns [`ReplCompiledChange`](../src/repl/compile.h#L132)) | [`src/repl/compile.c`](../src/repl/compile.c) |
...
per-pass data belongs in [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L395), for example camera world position,
...
the explicit [`Render3dGuideSnapshot`](../src/render3d/guides/guides_shared.h#L53) rather than pulling globals directly.
...
([`src/render3d/guides/geometry_guides.c`](../src/render3d/guides/geometry_guides.c)) render from a [`Render3dGuideSnapshot`](../src/render3d/guides/guides_shared.h#L53).
...
parsing uses [`repl_scan_next_arg_delim()`](../src/repl/eval.h#L443) so nested expressions such as
...
domain tags. Top-level visible rows are tags ([`repl_tutorial_visible_tag_count()`](../src/repl/tutorials.h#L354)
...
[`replay_exec_limit()`](../src/subsystems/replay/replay.h#L81): scrubbing a
...
**Several series.** Up to `MAX_ASSIGN_PLOT_SERIES` (4) rows plot together -
Shift+right-click adds or removes one, plain right-click retargets to a single
row. `series[0]` is the primary and it alone fixes the X axis, so a row whose
execution count disagrees with the mode is **refused when it is added**
(`ASSIGN_PLOT_SERIES_INCOMPATIBLE`), judged against the flat program as it
stands at click time; a row that has never executed cannot be judged and is
admitted. In `X_FRAME` mode the shared axis has to stay meaningful column by
column, so every series appends exactly one column per capture - a series that
did not run contributes `valid = 0` (a drawn gap, uncounted by the statistics)
and one added mid-history is back-filled with gaps rather than starting short
...
statistics are what carry the real numbers there.

**Ownership split.** The peer subsystem owns only the target lines, the rate,
the presentation chips, and per series the buffer and statistics. The panel
*titles* are not stored - the controller re-derives each from its live source
row's left-hand side every frame (`glr_ctrl_assign_plot_title`), so editing a
row retitles its series instead of stranding a stale label, and the subsystem
stays free of any `src/editor` dependency. The drift check is correspondingly
loose: a row must still exist and still be an assignment, not still be the
*same* assignment. A row that fails it drops that series alone; the panel
closes only when the last one goes.

**Controls are mouse-only, by design.** Capture rate (once / 1 Hz / every
frame), the Y scale and the zoom are properties of one plot, not global
settings, so none has a [`keymap.h`](../keymap.h) binding or a `GlrConfigKey` -
and therefore none reaches an exported `@cfg` header. Left-press on the rate
chip cycles forward, right-press backward, mirroring the Config flyout idiom;
the two-state chips do the same thing on either button. Right-clicking the
...
entry and the described one cannot disagree. Clicks there are consumed as inert
chrome. Panel size depends on `expanded` *and* `series_count` - the legend row
exists only past one series - so `ui_assign_plot_panel_size()` takes both and
...
env-capture run kind (`GLR_POINTER_SCRIPT`), distinguished by a [`PsRunKind`](../src/app/glr_pointer_script.c#L177)
...
* **Virtual clock vs rendered frames.** The host schedules
  ...
  stage drives [`glr_pointer_script_frame()`](../src/app/glr_pointer_script.h#L260),
...
* **Event accounting.** `g_next_event` / `g_current_event` / `g_completed_events`
  ...
  [`PsWait`](../src/app/glr_pointer_script.c#L235) completing does (immediate verbs complete the following virtual
...
* **HUD.** The controller populates `snap->tour` from
  [`glr_pointer_script_tour_view()`](../src/app/glr_pointer_script.h#L181) and renders
...
| **Controller router** | `glr_ctrl_keyboard`, `glr_ctrl_special`, and `glr_ctrl_router_*` in [`src/app/glr_ctrl_router.c`](../src/app/glr_ctrl_router.c) | Cross-subsystem actions: save, debug dump, quit, replay jump, accumulation samples, code focus, audio track, help, scene cycle, and modal close. |
...
Target rule: `render3d_*` files consume [`Render3dRenderConfig`](../src/render3d/render_types.h#L139), [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L395),
...
> [!WARNING]
> Detection MUST run before [`repl_apply_init_bootstrap()`](../src/repl/pipeline.h#L62) in the same
...
1. **Render3d caches what it applied (Tenet 3).**
   ...
   into a file static every frame; [`render3d_get_active_projection()`](../src/render3d/render.h#L145) reads
...
2. **Controller-installed projection bridge** (same shape as
   ...
   adapter reads [`render3d_get_active_projection()`](../src/render3d/render.h#L145) and formats the C
...
`RENDER3D_ORTHO_REF_FROZEN` mode; see [`config.h`](../config.h).)
...
non-default `RENDER3D_ORTHO_REF_PERFRAME` knob re-probes every ortho frame and
...
>   evaluated inline by [`src/repl/eval.c`](../src/repl/eval.c), never become a [`CmdType`](../src/repl/command.h#L48), and skip
...
`glVertex3f(rand2(t, 0), …)`) do **not** become a [`CmdType`](../src/repl/command.h#L48) and do **not**
...
Add a new [`CmdType`](../src/repl/command.h#L48) enum entry in the `CMD_*` block, adjacent to related
commands. The enum drives switch dispatch everywhere. ([`CmdType`](../src/repl/command.h#L48) lives in
...
Beyond the shared [`CmdType`](../src/repl/command.h#L48) + `command_spec` wiring already covered by the
...
6. **Round-trip: export / import / reformat.** Export can often emit canonical
   source text unchanged, but import must rebuild the same [`GLCmd`](../src/repl/command.h#L127) through
...
Tutorial step kinds ([`TutorialStepKind`](../src/repl/tutorials.h#L164) in [`src/repl/tutorials.h`](../src/repl/tutorials.h)) name the
...
| `NOTE`        | `comment` only              | Ack key (Enter / Tab / Space) - SET's showcase flow without the cfg write; the document stays frozen while it waits. Under label placement (`STEP_NOTE_AT`) it also becomes "go look at that row": the comment splices in above the labeled row and the cursor parks IN that row - focused, i.e. loaded into the input buffer the way arrowing onto it does, since the code panel draws the input buffer at the cursor and an empty one would hide the row. The target must be a COMMAND step or a setup `@anchor`; the comment-only kinds commit no row to park on. |
...
one matcher `repl_tutorial_anchor_name()` the catalog validator also
uses). Anchors are tutorial-private: an ordinary comment row, so flatten
drops them and no `CmdType` or parser grammar is involved. See
...
- Add the new enum value to [`TutorialStepKind`](../src/repl/tutorials.h#L164).
- Add any extra fields to [`TutorialStep`](../src/repl/tutorials.h#L199). **Place new fields AFTER all
...
- If the kind has no expected-commit-line dance (REQUIRE / REQUIRE_VAR),
  ...
  [`tutorial_note_expected_commit_applied()`](../src/subsystems/tutorial/tutorial.h#L170) returns 1 - i.e. a pending
...
- The catalog validator `expected_is_single_command` rejects
  `float ...;` declarations in a COMMAND `expected` (a CMD_VAR_DECLARE typed
  below existing code is relocated to the end of the declaration prologue by
