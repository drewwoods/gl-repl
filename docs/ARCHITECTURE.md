...
parsing uses [`repl_scan_next_arg_delim()`](../src/repl/eval.h#L438) so nested expressions such as
...
env-capture run kind (`GLR_POINTER_SCRIPT`), distinguished by a [`PsRunKind`](../src/app/glr_pointer_script.c#L135)
...
* **Virtual clock vs rendered frames.** The host schedules
  ...
  stage drives [`glr_pointer_script_frame()`](../src/app/glr_pointer_script.h#L237),
...
* **Event accounting.** `g_next_event` / `g_current_event` / `g_completed_events`
  ...
  [`PsWait`](../src/app/glr_pointer_script.c#L185) completing does (immediate verbs complete the following virtual
...
* **HUD.** The controller populates `snap->tour` from
  [`glr_pointer_script_tour_view()`](../src/app/glr_pointer_script.h#L158) and renders
...
| Seam | Purpose |
|---|---|
| Config bridge | [`glr_actions_install_export_cfg_bridge()`](../src/app/glr_actions.h#L127) exposes `@cfg` reads and writes without coupling export to app config modules. |
