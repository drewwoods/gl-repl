...
* **Special keys (F-keys / arrows):** replay → **cfg special (F2–F9)**
  → post-process filter (F10 / Shift+F10) → audio (Ctrl+Left/Right) →
...
* **Config Bridge:** Installed via [`glr_actions_install_export_cfg_bridge()`](../src/app/glr_actions.h#L102) so the exporter can read/write `@cfg` blocks without direct coupling to app configuration modules.
...
* New 3D REPL-aware overlay: current home is still `render3d_*`, consuming
  [`FlatProgramView`](../src/repl/flatten.h#L45) or a snapshot from [`Render3dRenderConfig`](../src/render3d/render_types.h#L131).
