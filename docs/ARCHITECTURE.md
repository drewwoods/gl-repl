...
> `src/repl` interpreter — the [`GLCmd`](../src/repl/command.h#L87) record and provenance, the
...
assignments against the current bindings, and stores resolved [`GLCmd`](../src/repl/command.h#L87) records
...
[`GLCmd`](../src/repl/command.h#L87) is a pure parse-result struct: `type`, `args[]`, validity / vars
...
[`ReplRuntimeState`](../src/repl/state.h#L18). The parser returns both the [`GLCmd`](../src/repl/command.h#L87) and the canonical
...
* build [`Render3dRenderConfig`](../src/render3d/render_types.h#L136) and any guide/focus snapshots from REPL state
...
[`Render3dRenderConfig`](../src/render3d/render_types.h#L136) is the render3d module's explicit per-frame input. It may carry
...
per-pass data belongs in [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L306), for example camera world height,
...
membership predicate is [`repl_cmd_is_glut_solid()`](../src/repl/command.h#L190) in [`src/repl/command.h`](../src/repl/command.h)
...
[`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L102) walks the current [`FlatProgramView`](../src/repl/flatten.h#L45) via
...
command, [`cursor_guide_snapshot_with_flat_args()`](../src/subsystems/edit_overlays/edit_overlays.h#L112) replaces `vertex_args` or
...
That independence is what makes [`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L102) flush the
...
* accumulation-AA settings are [`Render3dRenderConfig`](../src/render3d/render_types.h#L136) fields set by the controller
...
Target rule: `render3d_*` files consume [`Render3dRenderConfig`](../src/render3d/render_types.h#L136), [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L306),
...
setters and [`Render3dRenderConfig`](../src/render3d/render_types.h#L136), never re-queried per frame.
...
* **Code panel (per frame):** the controller resolves the block once in
  ...
  [`Render3dRenderConfig`](../src/render3d/render_types.h#L136)). The block is the *previous* frame's render3d
...
[`Render3dRenderConfig`](../src/render3d/render_types.h#L136).
...
6. **Round-trip: export / import / reformat.** Export can often emit canonical
   source text unchanged, but import must rebuild the same [`GLCmd`](../src/repl/command.h#L87) through
