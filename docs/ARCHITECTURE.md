...
> `src/repl` interpreter — the [`GLCmd`](../src/repl/command.h#L97) record and provenance, the
...
assignments against the current bindings, and stores resolved [`GLCmd`](../src/repl/command.h#L97) records
...
[`GLCmd`](../src/repl/command.h#L97) is a pure parse-result struct: `type`, `args[]`, validity / vars
...
[`ReplRuntimeState`](../src/repl/state.h#L18). The parser returns both the [`GLCmd`](../src/repl/command.h#L97) and the canonical
...
per-pass data belongs in [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L312), for example camera world height,
...
membership predicate is [`repl_cmd_is_glut_solid()`](../src/repl/command.h#L215) in [`src/repl/command.h`](../src/repl/command.h)
...
[`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L142) walks the current [`FlatProgramView`](../src/repl/flatten.h#L46) via
...
command, [`cursor_guide_snapshot_with_flat_args()`](../src/subsystems/edit_overlays/edit_overlays.h#L152) replaces `vertex_args` or
...
That independence is what makes [`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L142) flush the
...
Target rule: `render3d_*` files consume [`Render3dRenderConfig`](../src/render3d/render_types.h#L135), [`Render3dFrameRenderContext`](../src/render3d/render_types.h#L312),
...
| Seam | Purpose |
|---|---|
| Config bridge | [`glr_actions_install_export_cfg_bridge()`](../src/app/glr_actions.h#L119) exposes `@cfg` reads and writes without coupling export to app config modules. |
...
[`glr_ctrl_display_frame()`](../src/app/glr_ctrl.h#L152) (snapshot/follow-scroll → render3d render →
...
6. **Round-trip: export / import / reformat.** Export can often emit canonical
   source text unchanged, but import must rebuild the same [`GLCmd`](../src/repl/command.h#L97) through
