...
> `src/repl` interpreter — the [`GLCmd`](../src/repl/command.h#L110) record and provenance, the
...
assignments against the current bindings, and stores resolved [`GLCmd`](../src/repl/command.h#L110) records
...
[`GLCmd`](../src/repl/command.h#L110) is a pure parse-result struct: `type`, `args[]`, validity / vars
...
[`ReplRuntimeState`](../src/repl/state.h#L18). The parser returns both the [`GLCmd`](../src/repl/command.h#L110) and the canonical
...
membership predicate is [`repl_cmd_is_glut_solid()`](../src/repl/command.h#L246) in [`src/repl/command.h`](../src/repl/command.h)
...
[`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L150) walks the current [`FlatProgramView`](../src/repl/flatten.h#L46) via
...
[`cursor_guide_snapshot_with_flat_args()`](../src/subsystems/edit_overlays/edit_overlays.h#L160) replaces `vertex_args` or
...
That independence is what makes [`edit_overlays_render_cursor_guides()`](../src/subsystems/edit_overlays/edit_overlays.h#L150) flush the
...
* New 3D REPL-aware overlay: current home is still `render3d_*`, consuming
  [`FlatProgramView`](../src/repl/flatten.h#L46) or a snapshot from [`Render3dRenderConfig`](../src/render3d/render_types.h#L140).
...
6. **Round-trip: export / import / reformat.** Export can often emit canonical
   source text unchanged, but import must rebuild the same [`GLCmd`](../src/repl/command.h#L110) through
