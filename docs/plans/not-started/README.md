...
| `makefile-target-conventions.md` | Make target grammar (`<verb>-<subject>`, 14 verbs, complete before/after inventory), the orthogonal `PLATFORM`/`GL_BACKEND`/`FORMAT` variables that replace the `-web`/`-csv` target families, three-tier generated help, and six `check-make-*` guards enforcing it |
