...
| `repl-capability-gaps.md` | Prioritized index of the REPL's language + GL-surface gaps, derived from the stencil-shadow-volume scene: what each unlocks, effort, and sequencing (indexes `float-returning-repl-functions.md` and `stencil-buffer-support.md` Phase 3 rather than restating them) |
