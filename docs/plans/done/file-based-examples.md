...
into the binary through generated C data, so runtime behavior stays unchanged:
`--example`, F12 cycling, app bundles, and tests do not read `examples/` at
runtime.
...
- `examples/scenes/` contains the authored built-in scene snippets.
...
[animated-ring-for-t]
file = scenes/animated-ring-for-t.c
...
- Scene files keep only runtime scene metadata: leading `// @cfg ...`
  presentation lines, optional `// camera`, then REPL source.

## Implementation Notes

`src/repl/examples.h` stayed API-compatible. `src/repl/examples.c` is now a
small query facade that keeps the tag labels and synthetic `All` behavior, then
includes generated example source arrays and the generated
`ReplExampleEntry g_example_entries[]` table.
...
`examples/scenes/*.c`, preserving source lines, comments, blank lines, `@cfg`
presentation settings, and camera blocks.
...
- `tests/test_repl_core_examples.c` now checks catalog metadata integrity.
...
The live demo informed the repository shape, but built-in examples continue to
use the existing compiled-in catalog and loader path.
