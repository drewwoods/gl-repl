...
|---|---|---|
| [`warn-when-expensive-setting-is-costing-frames.md`](warn-when-expensive-setting-is-costing-frames.md) | 2026-08-28 | Code-panel status-strip warning when accum passes, Post FX, or line smooth is above its shipped default and the frame rate stays below a display-relative threshold for 2 s. Names the heaviest culprit, `[off]` steps it back, `[x]` dismisses for the session. Tours and `FREEGLUT_CAPTURE_*` runs suppress it. MSAA is out of the first cut. |
| [`router-complexity-reduction.md`](router-complexity-reduction.md) | 2026-08-21 | `glr_ctrl_router.c` 3,313 -> 2,822 lines, reduced to what its header claimed: routing. Three non-routing bands evicted to modules that own their domain - scene/tutorial cycling to `glr_ctrl_cycle.c` (incl. the stepper-tooltip prediction, so it cannot drift from the walk it mirrors), quit recovery to `glr_ctrl_recovery.c`, the variable-panel drag's document writes to a new `glr_variable_panel_bridge.{c,h}` that also absorbed the read-side installer. The press hit-test order - previously walked twice, once to route and once to predict - is stated once in `ui_panels_hit_test_layered()` beside the paint order it must match, which settled a live disagreement between the two copies. Dropdown-dismiss exemptions became a named positive list with each member's reason recorded; the shared key-dispatch prologue became one `router_modal_capture()`. Stage 3's 45-arm dispatch table and Stage 5's by-surface TU split were **rejected** - see the plan's status header. |
