...
## Status - PROJECT 1 COMPLETE; INTERACTIVE HOIST FIXED; PROJECT 2 REMAINDER NOT APPROVED; FILE-SCOPE INSERT SLOT DESIGNED (REV 5), NOT IMPLEMENTED (2026-08-25)
...
| Explicit insertion above `display()` | Add a separate synthetic file-scope slot immediately before the generated display opener. It accepts float declarations and function definitions (revision 5 - comments removed per forward-attachment rule). It does not replace the existing boundary ghost, which remains the first display-body insertion row. Design specified below, revision 5; not implemented. |
...
## Explicit File-Scope Insertion Slot (design approved, revision 5; not implemented)
...
**Revision 5** closed all review blockers:
- **Structural-edit-aware boundary re-indentation**: `repl_reindent_after_change(at_before, change)` maps post-change indices back to pre-change coordinates using change offsets, preventing double-indentation of shifted body rows on deletions or multi-row insertions (R5).
- **Slot restricted strictly to floats and functions**: Standalone comments are removed from the slot because the boundary's forward-attachment rule assigns a comment run to the following body command inside `display()`, and a subsequent function inserts before it rather than attaching to it. The placeholder is `+ float or function` and the diagnostic is `"File scope accepts only float declarations or function definitions"` (R5).
- **Navigation transactions**: Up/Down and mouse transitions route through editor-owned `editor_input_enter_file_scope_slot()` and `editor_input_exit_file_scope_slot()`, executing `commit_before_navigation()` so pending edits are committed or rolled back on failure (R5).
- **Early dispatch gate**: `editor_handle_key` blocks Ctrl+V (paste) and Ctrl+/ (comment toggle) at the top of the chain when in `EDITOR_INSERT_FILE_SCOPE`, preventing bypass of file-scope restrictions (R5).
- **Scope lifetime invariant**: `insert_scope` resets to `EDITOR_INSERT_DOCUMENT` whenever `edit_line` changes or `insert_mode` becomes 0 (R5).
- **Snapshot purity**: UI code reads `snap->tutorial.active` on `UiRenderSnapshot` rather than calling `tutorial_active()` directly (R5).
...
**R5 - Scope lifetime invariant:** `insert_scope` is cleared to
`EDITOR_INSERT_DOCUMENT` whenever `edit_line` changes or `insert_mode` becomes
0. `EDITOR_INSERT_FILE_SCOPE` is valid *only* while `insert_mode == 1` and
`edit_line == display_body_start`. Escape, ordinary navigation, undo/redo
restore, scene/workspace switches and any transition out of insert mode reset
the scope to `EDITOR_INSERT_DOCUMENT`. The scope is transient editor state; it
is not part of the REPL document and does not alter import, paste,
external-editor row maps or saved files.
...
**R4/R5 - keyboard entry & stated navigation decision.** `make keymap-list` reports
zero free plain-Ctrl slots and zero free F-keys; only `Ctrl+Shift+M/Q/Y`
remain. **Up-arrow from the first display-body row enters the slot**, which is
the inverse of the navigation rule that leaves it. That costs no keymap slot
and keeps the affordance discoverable. This is an intentional navigation
decision: stepping from the first body row to the last prologue row takes two
Up-arrow presses (first into the slot, then into the prologue row). Down-arrow
(or any other navigation) exits per the rule above. In a functions-only document
where `display_body_start == document_count` and no body commands exist,
Up-arrow from the trailing input/placeholder row enters the slot.

**R5 - editor-owned navigation transaction.** The controller router and keyboard
navigation do not mutate editor fields directly. They call editor-owned
`editor_input_enter_file_scope_slot()` and `editor_input_exit_file_scope_slot()`,
which run `commit_before_navigation()`. If a pending commit fails, navigation is
rejected and the slot/cursor/input state is held in place.
...
`src/app/glr_ctrl_router.c` routes that hit to `editor_input_enter_file_scope_slot()`.
Generated `display()` chrome itself remains inert.

**R4 - reverse mapping disambiguation:** `ui_repl_code_panel_target_for_doc_line()`
(`src/ui/app/repl_code_panel.h:131`) takes an optional `EditorInsertScope *out_insert_scope`
to distinguish whether a resolved insert row at `display_body_start` is the
file-scope slot (`EDITOR_INSERT_FILE_SCOPE`) or the body insert row
(`EDITOR_INSERT_DOCUMENT`).

The row should exist in both full and code-focus views. When inactive it renders
as a dim `+ float or function` placeholder; when active it renders the
ordinary input buffer at column zero. Hide or disable it during an active
tutorial (`snap->tutorial.active` checked on the snapshot, preserving UI purity):
tutorial clear-prelude rows are locked, and moving a function above them
remains the gated Project 2 remapping problem.

### Restricted commit chain

**R5 - early dispatch gate:** In `keyboard_func()` (at the top of
`editor_handle_key`), when `editor_insert_scope() == EDITOR_INSERT_FILE_SCOPE`,
intercept `handle_paste_key_route` (Ctrl+V) and `handle_comment_toggle_key_route`
(Ctrl+/), rejecting them with `"File scope accepts only float declarations or function definitions"`.
This prevents paste or comment-toggle from bypassing the restricted commit
chain.

Both Enter and the semicolon commit key intercept `EDITOR_INSERT_FILE_SCOPE`
before the ordinary commit path. The only handlers offered are, in order:

1. `repl_compile_float_decl()`;
2. `editor_compile_func_def()`.

There is no fallthrough to assignments, `if`/`for`/close-brace handlers, the
general GL parser, blank-line insertion, comments, or paste. Any other input is
consumed as an error with the direct diagnostic:
`File scope accepts only float declarations or function definitions`, leaving
the input and scoped cursor in place.

**R4/R5 - commit success signal & rejection handling:** `editor_try_commit_block()`
returns 1 on success and 1 on handled compile rejections (e.g. duplicate name or
overflow). The slot resolves success by inspecting `plan.change.kind` (or using
checked compile/apply helpers):
- After a float declaration that actually applied, keep the scoped slot active
  and retarget it to the recomputed `display_body_start`, allowing consecutive
  declarations. Plain `float` and `static float` use the existing top-level
  declaration semantics; assignments remain forbidden.
- After a function header that actually applied, reset to
  `EDITOR_INSERT_DOCUMENT` and preserve the existing function commit post-effect:
  the matching `}` is inserted automatically and the cursor lands inside the
  function body, where ordinary commands are legal.
- A *rejected* commit of either kind holds the slot exactly as it was - scope,
  cursor, and input text - so the diagnostic can be acted on in place.

Commit-time restriction is the safety boundary; autocomplete filtering is a
secondary usability improvement. In this scope completion in
`src/app/glr_completion.c` matches `float`, `static float`, and function-header
guidance while suppressing GL calls that the commit path will reject.
...
### Structural-edit-aware boundary re-indentation (R5)

A row's base indent is a whole-document property: 0 above the boundary, 2
below. Anything that moves `display_body_start` past an existing row owes that
row a re-indent.

Calling `repl_reindent_after_boundary_move()` unconditionally with a bare
`at_before` scalar is unsafe for structural insertions and deletions because it
compares post-change row indices directly against `at_before`. For example,
deleting a two-row function changes the boundary from 4 to 2 and shifts the
first body row from index 4 to index 2; the naive helper sees post-change index 2
as formerly file-scope and adds two spaces, double-indenting the body row.

**R5 resolution:** `repl_reindent_after_change(int at_before, const ReplCompiledChange *change)`
in `src/repl/reformat.c` maps each post-change row index `cmd_idx` back to its
pre-change index `pre_idx` using `change->pos`, `change->count`,
`change->delete_pos`, and `change->delete_count`. For existing rows:
- `was_body = pre_idx >= at_before;`
- `is_body  = cmd_idx >= at_after;`
- If `was_body != is_body`, shift row indentation by
  `delta = (is_body ? 2 : 0) - (was_body ? 2 : 0)`.

Call `repl_reindent_after_change(at_before, change)` from `apply_compiled_change_full()`
in `src/editor/commit.c`, capturing `at_before` prior to the mutation. This
safely re-indents comment runs hoisted by declarations without corrupting
shifted rows on deletions or multi-row inserts.
...
- Consecutive float declarations keep the slot active at the recomputed
  ...
  body.
- GL calls, assignments, blank input, comments, `if`, `for`, `}` and paste are
  rejected without document mutation or generic-parser fallthrough, with
  diagnostic `"File scope accepts only float declarations or function definitions"`.
- Escape, click/arrow navigation, undo/redo and scene changes clear the scoped
  state.
- The slot is absent or inert during tutorials (`snap->tutorial.active`), so
  locked-row indices and expected-commit mapping do not move.
...
- **A rejected commit holds the slot:** A duplicate `funcN`, duplicate var name,
  or overflow leaves scope, cursor, and input text untouched and the document
  unmutated.
- **Pending-input navigation transaction:** Entering or exiting via Up-arrow,
  Down-arrow, or mouse hit runs `commit_before_navigation()`, committing dirty
  input or aborting navigation on rejection.
- **Early dispatch gate:** Ctrl+V paste and Ctrl+/ comment-toggle are blocked
  when `insert_scope == EDITOR_INSERT_FILE_SCOPE`.
- **Functions-only keyboard entry:** When `display_body_start == document_count`,
  Up-arrow from the trailing input/placeholder row enters the slot.
- **Structural-edit-aware re-indentation:** `repl_reindent_after_change()`
  re-indents comment-led declaration hoists without double-indenting rows on
  function deletion.
- **Reverse mapping disambiguation:** `ui_repl_code_panel_target_for_doc_line()`
  distinguishes the two insert rows sharing `display_body_start`.
- **Extedit input-row gate:** In `tests/test_glr_extedit.c`, verify a `--watch`
  session defers and resolves external changes while the slot is active, and
  entering/leaving the slot trips no outbound sync.
- **`EditorInputView` carries `insert_scope`** with updated contract comments.
