...
| `GLR_OPEN_COMMAND_HELP` | 0-based line index of a committed GL-family row. | Right-clicks that row to raise its authored help card; same retry-until-on-screen behavior. No-op on a row with no description record. Pose it *after* `GLR_OPEN_ASSIGN_PLOT` — a right-click on an assignment row closes the card. |
