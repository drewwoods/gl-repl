...
scripts/docs-assets.sh --gifs       # regenerate every regular GIF
scripts/docs-assets.sh --pngs       # regenerate every regular PNG
scripts/docs-assets.sh --perf-sensitive # explicit-only; unloaded machine
scripts/docs-assets.sh -j 4         # regenerate regular assets, four workers
scripts/docs-assets.sh hero replay  # regenerate selected assets
scripts/docs-assets.sh profile-panels # the same asset selected by name
scripts/docs-assets.sh --help       # full CLI reference
```

The `--perf-sensitive` category is excluded from default, `--gifs`, and
`--pngs` runs. It currently contains `profile-panels`, which captures a live
performance profile. Regenerate it explicitly on an otherwise unloaded machine
so the profile is representative.
