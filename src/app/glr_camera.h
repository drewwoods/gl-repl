...
void             glr_camera_ease_to(float rx, float ry, float dist,
                                    float tx, float ty, float tz);
/* Re-aim an in-flight (or fresh) ease so the world-space bounds fit the
 * viewport described by fovy/aspect, keeping the current orientation target.
 * Returns 1 when it starts an ease, 0 for invalid / degenerate bounds. */
