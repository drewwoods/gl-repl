 ...
 * exclusive by Post FX Scope, so they never both run in one frame; the
 * shared scratch texture inside postprocess_filter.c is therefore never
 * contended, and even if both ran it would only grow to the larger
 * (window) rect and stay correct.
 */
