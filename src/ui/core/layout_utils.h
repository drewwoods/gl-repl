...
static inline int ui_clamp_panel_y(int render3d_y, int render3d_h, int panel_h,
                                   int requested_y,
                                   int prefer_top_on_overflow,
                                   int bottom_inset, int edge_pad) {
    int min_y = render3d_y + bottom_inset + edge_pad;
    int max_y = render3d_y + render3d_h - panel_h - edge_pad;
    if (max_y >= min_y) {
    ...
    return prefer_top_on_overflow
         ? (render3d_y + render3d_h - panel_h - edge_pad)
         : min_y;
}
