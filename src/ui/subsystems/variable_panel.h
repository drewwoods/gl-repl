...
typedef struct {
    ...
    int          is_bool; /* 1 if @bool-tagged: the row draws a checkbox and
                           * YES/NO value instead of a numeric slider value,
                           * and a click writes 1 or 0 rather than starting a
                           * scrub. The variable is still a float; "on" is
                           * value > 0.5 */
