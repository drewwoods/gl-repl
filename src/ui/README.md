...
- **A UI renderer may draw.** It takes a read-only snapshot
  ([`UiRenderSnapshot`](app/snapshot.h#L71)) and produces pixels. It does not read live program or
...
| [`app/snapshot.h`](app/snapshot.h) | [`UiRenderSnapshot`](app/snapshot.h#L71) — the read-only per-frame bundle every renderer takes |
