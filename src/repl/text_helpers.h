...
/* One `%f` substitution in console() is this many characters.
...
/* Format a console() payload with fixed-width numeric fields.
 * Walks `fmt`, expanding `%f` from `args[0..num_args-1]` as a
 * REPL_CONSOLE_FLOAT_WIDTH-character field, and `%%` to literal '%'.
...
/* Format a bitmap label payload. `%f` is the REPL language's only numeric
 * placeholder, but labels use compact `%g` text rather than console's stable
 * columns. Exported label() uses the same representation. */
