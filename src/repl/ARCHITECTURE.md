...
|---|---|
| Lexer / parser → AST | [`parser.c`](parser.c) → [`GLCmd`](command.h#L127) records |
...
### 3.1 [`GLCmd`](command.h#L127) - the universal command record

A single [`GLCmd`](command.h#L127) ([`command.h`](command.h)) represents one command in *either* level.
...
[`CmdType`](command.h#L48) ordering is **append-stable**: switch dispatch in [`executor.c`](executor.c),
...
the live lexical bindings into a parallel [`FlatCmdLocalVars`](flatten.h#L79) array
([`repl_state_flat_program_local_vars()`](state_views.h#L202)):
...
[`FlatCmdLocalVars`](flatten.h#L79) values for that match: those values are effective command
...
[`command_store.c`](command_store.c) underneath is the lowest layer: pure [`GLCmd`](command.h#L127) array
...
[`repl_flatten_program()`](flatten.h#L240) ([`flatten.c`](flatten.c)) expands the source array into the
flat array:
...
provenance (§3.3) and a [`FlatCmdLocalVars`](flatten.h#L79) snapshot (§3.4).
...
paths. They all produce the same baked [`GLCmd`](command.h#L127) stream:
...
1. A command whose `has_vars` is 0 is appended verbatim from the committed
   ...
   frames. The enclosing [`FlatCmdLocalVars`](flatten.h#L79) snapshot is still
...
| One [`repl_flatten_program()`](flatten.h#L240) call | Set `ReplFlattenOptions.expr_cache = NULL` | Same cache-free direct/text behavior, without changing the live cache or other callers. |
...
buffer and pass a [`FlatProgramView`](flatten.h#L59) over it, never touching
the live arrays.
...
- **Matrix-stack tracking.** `repl_executor_apply_tracked_transform_cmd`
  ...
  geometry by transform depth. The GL matrix stack - not [`GLCmd`](command.h#L127) - is the
...
| [`ReplVariableState`](state_views.h#L94) | predef var table, scratch arrays `A/B/C`, `funcN` aliases, the `t` clock (`anim_time`, `time_playing`) |
| [`ReplRenderState`](state_views.h#L133) | the runtime-mutated render *tail*: `light_enabled_mask` alone. The frame background is not state here - the cursor observes it while emitting the program's clears and publishes a [`ReplBackgroundObservation`](executor.h#L64) |
| [`ReplSceneRuntimeState`](state_views.h#L164) | active example index, bound workspace dir |
| [`ReplImportExportState`](state_views.h#L174) | cached header/render/camera text + pending import metadata |
...
| `return;` | `CMD_RETURN` | nothing - a successful early-exit signal consumed at the current function or frame boundary |
...
**Jumps** are the constructs that flatten to nothing at all. `CMD_BREAK` /
`CMD_CONTINUE` raise `FlattenContext.loop_signal` and return; every walk that
can sit between the statement and its loop unwinds on it, and the innermost
`flatten_for_loop` consumes it - *after* copying loop-body variable writes back,
so an assignment that ran before the break is not lost. `CMD_RETURN` uses the
same unwind path, but passes through enclosing loops and is consumed
successfully at the current function boundary (or at the top-level frame
boundary). A surviving `BREAK` / `CONTINUE` at `flatten_call` is instead a
stale invalid state and fails the frame, because a callee's loop jump must not
reach the caller's loop. Because jumps never reach the flat program, the
executor's arms are no-ops, replay never steps them, and no dep bookkeeping is
needed - the guarding `if` condition is already noted structural, which is
what forces a re-flatten rather than a value-only rebake when the guard's
inputs change.
...
[`repl_flatten_program()`](flatten.h#L240) (§5.2) lowers the nine source commands to eleven
