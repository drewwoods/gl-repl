...
|---|---|
| Lexer / parser → AST | [`parser.c`](parser.c) → [`GLCmd`](command.h#L110) records |
...
### 3.1 [`GLCmd`](command.h#L110) — the universal command record

A single [`GLCmd`](command.h#L110) ([`command.h`](command.h)) represents one command in *either* level.
...
[`command_store.c`](command_store.c) underneath is the lowest layer: pure [`GLCmd`](command.h#L110) array
...
paths. They all produce the same baked [`GLCmd`](command.h#L110) stream:
...
- **Matrix-stack tracking.** `repl_executor_apply_tracked_transform_cmd`
  ...
  geometry by transform depth. The GL matrix stack — not [`GLCmd`](command.h#L110) — is the
