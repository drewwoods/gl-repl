...
`glBegin`/`glVertex3f`/`glVertex4f`/`glColor3f`/`glRotatef`/… - plus light control flow
...
- On success the editor applies the change to REPL runtime state via
  `repl_apply_*`, and `repl_command_store` does the low-level [`GLCmd`](command.h#L127) array
  shuffling.
...
[`GLCmd`](command.h#L127) is a pure parse result (type, args, flags, provenance) - it carries
...
| File | Responsibility |
|---|---|
| [`command.h`](command.h) | Core types: [`CmdType`](command.h#L48), [`GLCmd`](command.h#L127), control-flow predicates |
...
| [`parser.c`](parser.c) / `.h` | One source line → [`GLCmd`](command.h#L127) + canonical text |
...
| [`command_store.c`](command_store.c) / `.h` | Low-level [`GLCmd`](command.h#L127) array mechanics (insert/replace/delete/load) |
...
| [`doc_order.c`](doc_order.c) / `.h` | The canonical `.glr` document order as a monotonic four-phase machine (decls → funcs → camera → body). Every scene makes the funcs/body transition explicit with exactly one `display() { ... }` frame. `.glr` only; **every loader must run it** or the format has two readings. Names both the offending line and the line that set the phase, and reports the whole file in one pass (`ARCHITECTURE.md` §4.6) |
