...
- **Generic, application-free core.** `state.c` owns the document
  (`EditorState`: buffer, cursor, selection, scroll, undo, …) and
  `edit_ops.c` provides the primitive operations (insert a char, delete at
  cursor, consume a selection) that *any* plain-text editor needs. These
  know nothing about OpenGL, REPL grammar, or this app.
- **Application-flavored controllers.** `input.c` (key/mouse dispatch with
  this app's bindings), `commit.c` (the transaction that turns a text edit
  into a validated program change), `clipboard.c`, `undo.c`, `search.c`,
  `completion.c`, and the inline overlays. These encode *policy* — what a
...
The demo's value is what it *refuses* to link: `input.c`, `commit.c`,
`clipboard.c`, `undo.c`, `reformat.c`, `search.c`, and `completion.c` are all
...
- The editor exposes per-frame snapshots that `src/ui` renders, and it
  consumes neutral `UiHit` results (from UI hit-testing) to interpret mouse
...
- A keystroke that can change line text, cursor, scroll, selection,
  ...
  `src/app/glr_ctrl.c` has already filtered out non-editor concerns (replay,
...
The editor owns the **canonical per-line text**; `GLCmd` in `src/repl`
...
| File | Responsibility |
|---|---|
| `state.c` / `.h` | Owns `EditorState`: buffer, input, cursor, selection, scroll, search, autocomplete, undo, cursor blink |
| `edit_ops.c` / `.h` | Generic text-editing primitives (REPL-free; shared with `editor_demo`) |
| `input.c` / `.h` | REPL editor key/mouse dispatcher (`;` commit, Tab, Ctrl+R, comment toggle, …) |
| `commit.c` / `.h` | Commit transaction boundary: compile → undo → buffer write → REPL apply |
| `undo.c` / `.h` | Undo/redo rings (restore editor text + REPL command state together) |
...
| `reformat.c` / `.h` | Whole-document reindent (Ctrl+R) |
| `limits.h` | Shared editor input / autocomplete capacity constants |
