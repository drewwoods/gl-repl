...
- **Generic, application-free core.** [`state.c`](src/editor/state.c) owns the document
  ([`EditorState`](src/editor/state.h#L175): buffer, cursor, selection, scroll, undo, …) and
  [`edit_ops.c`](src/editor/edit_ops.c) provides the primitive operations (insert a char, delete at
  cursor, consume a selection) that *any* plain-text editor needs. These
  know nothing about OpenGL, REPL grammar, or this app.
- **Application-flavored controllers.** [`input.c`](src/editor/input.c) (key/mouse dispatch with
  this app's bindings), [`commit.c`](src/editor/commit.c) (the transaction that turns a text edit
  into a validated program change), [`clipboard.c`](src/editor/clipboard.c), [`undo.c`](src/editor/undo.c), [`search.c`](src/editor/search.c),
  [`completion.c`](src/editor/completion.c), and the inline overlays. These encode *policy* — what a
...
[`state.c`](src/editor/state.c) (the document model) and [`edit_ops.c`](src/editor/edit_ops.c) (the primitives), plus the
...
code-panel adapter, menu bar, [`UiRenderSnapshot`](src/ui/app/snapshot.h#L70), [`UiState`](src/ui/app/state.h#L20), and app chrome are
...
The demo's value is what it *refuses* to link: [`input.c`](src/editor/input.c), [`commit.c`](src/editor/commit.c),
[`clipboard.c`](src/editor/clipboard.c), [`undo.c`](src/editor/undo.c), [`reformat.c`](src/editor/reformat.c), [`search.c`](src/editor/search.c), and [`completion.c`](src/editor/completion.c) are all
...
- The editor exposes per-frame snapshots that `src/ui` renders, and it
  consumes neutral [`UiHit`](src/ui/core/hit.h#L51) results (from UI hit-testing) to interpret mouse
...
- A keystroke that can change line text, cursor, scroll, selection,
  ...
  [`src/app/glr_ctrl.c`](src/app/glr_ctrl.c) has already filtered out non-editor concerns (replay,
  audio, config, save, camera, the variable panel).
- **Commits are transactions.** [`commit.c`](src/editor/commit.c) is the only path that crosses
...
- Read-only documents are also editor sessions: [`help_session.c`](src/editor/help_session.c) backs the
  F1 overlay with the same scroll/search/cursor model and no commit path.

The editor owns the **canonical per-line text**; [`GLCmd`](src/repl/command.h#L87) in `src/repl`
...
| File | Responsibility |
|---|---|
| [`state.c`](src/editor/state.c) / `.h` | Owns [`EditorState`](src/editor/state.h#L175): buffer, input, cursor, selection, scroll, search, autocomplete, undo, cursor blink |
| [`edit_ops.c`](src/editor/edit_ops.c) / `.h` | Generic text-editing primitives (REPL-free; shared with `editor_demo`) |
| [`input.c`](src/editor/input.c) / `.h` | REPL editor key/mouse dispatcher (`;` commit, Tab, Ctrl+R, comment toggle, …) |
| [`commit.c`](src/editor/commit.c) / `.h` | Commit transaction boundary: compile → undo → buffer write → REPL apply |
| [`undo.c`](src/editor/undo.c) / `.h` | Undo/redo rings (restore editor text + REPL command state together) |
| [`clipboard.c`](src/editor/clipboard.c) / `.h` | Selection anchors, copy/cut/paste payloads (line-range + input-text) |
| [`search.c`](src/editor/search.c) / `.h` | Case-insensitive search query + match navigation |
| [`completion.c`](src/editor/completion.c) / `.h` | Completion-provider registry (provider itself is [`src/app/glr_completion.c`](src/app/glr_completion.c)) |
| [`help_session.c`](src/editor/help_session.c) / `.h` | Read-only editor session for the F1 overlay (tab + scroll) |
| [`inline_rename.c`](src/editor/inline_rename.c) / `.h` | Inline scene-rename input buffer |
| [`inline_file_prompt.c`](src/editor/inline_file_prompt.c) / `.h` | Inline save/load filename prompt |
| [`reformat.c`](src/editor/reformat.c) / `.h` | Whole-document reindent (Ctrl+R) |
| [`limits.h`](src/editor/limits.h) | Shared editor input / autocomplete capacity constants |
